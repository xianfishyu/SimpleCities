...
public sealed class RoadGraphV3Application
{
    ...
    public V3ManifestFile? GetSlotFile(string slotId, string fileName) =>
        GetManifest(slotId)?.Files
            .FirstOrDefault(file => string.Equals(file.Name, fileName, StringComparison.Ordinal));
    ...
    public int GetSlotRoadNetworkPayloadLength(string slotId) =>
        GetSlotRoadNetworkPayload(slotId)?.Length ?? 0;

    public int GetSlotRoadNetworkJsonLength(string slotId) =>
        GetSlotRoadNetworkJson(slotId)?.Length ?? 0;

    public bool GetSlotRoadNetworkHasFile(string slotId) =>
        GetSlotRoadNetworkFile(slotId) is not null;

    public bool GetSlotThumbnailHasFile(string slotId) =>
        GetSlotThumbnailFile(slotId) is not null;

    public long? GetSlotThumbnailSize(string slotId)
    {
        string? thumbnailFile = GetSlotThumbnailFile(slotId);
        return thumbnailFile is null ? null : GetSlotFile(slotId, thumbnailFile)?.EncodedLength;
    }

    public V3SlotSummary GetSlotSummary(string slotId) => GetStatus(slotId);

    public bool GetSlotIsComplete(string slotId) =>
        GetSlotOccupant(slotId) == V3SlotOccupant.CompleteV3;

...
> 完成判定（2026-08-04）：上述条件全部满足，第二代道路系统验收完成。后续规范 Edge/环路、RoadType、交通模拟和高程道路必须作为第三代或更晚范围重新立项，不重新打开第二代完成状态。canonical Edge、自环/平行 Edge、RoadGroup 移除、RoadType、分级表现和改造现已在 [第三代道路系统迭代指南](road-system-v3-gen.md) 重新立项；第三代允许完全重写架构并采用独立保存根和新 format v1，不读取或迁移 V2 存档。交通模拟和高程道路仍不属于第三代。

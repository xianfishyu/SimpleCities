...
> 文档状态：实施中；Phase 1～4 与 Phase 6 已完成，Phase 5 的 `v3-save-system:2.1`～`2.2` 已完成、`2.3` 部分实现，Phase 7 的 `v3-grid-rendering:2.1`、分级 ribbon、六分量表现 token、同代 `EdgeRibbon` + `TerminalCap` surface、canonical `RoadLocation` 与基础 surface 空间索引切片已完成且 Load 协作参与者部分接入；当前入口为补齐 semantic join、junction patch 与工具接管
>
> 编写日期：2026-08-14
>
...
> 当前实施记录（2026-08-14）：Phase 1～4 已完成 mutation 数值/容量门禁、endpoint-role incidence、self-loop/parallel Edge、六类原生方向与权威锚、最大连续 Edge、半开 query fragment locality、RoadGroup 移除、closed/self-intersection、四类 `RoadType`、原子 `ChangeRoadType`、不可变 `RoadGraphRevision` root、统一 `GraphChanged`/可逆 delta，以及 lineage/domain revision/change sequence 身份。Phase 5 已完成独立 `user://saves-v3` 根、严格 `simple-cities-v3` format v1、有界 streaming reader、同句柄完整性校验、PNG 预算、五类 occupant、publish/delete descriptor、tombstone、OS 根锁和 digest 恢复矩阵；`SaveOperationCoordinator`、结构化 token/state/result、手动优先与单 pending autosave、取消/退出收敛，以及 RoadGraph + 空工具/history + renderer mesh/ribbon-cap surface + 槽目标的 aggregate Load 已实现，等待 gate 时外部取消不会再产生占锁 lease。Phase 6 的 delta/token 与 entry/估算字节双预算已完成。
>
> Phase 7 已部分实现普通与 Load 共用的 closed ribbon、循环 seam join、纯 self-loop seam marker 隐藏，以及两路口环、八字形和删除支路后的 seam 重定位；`v3-grid-rendering:2.1` 已完成四类 `RoadTypeStyle` 资源、严格唯一覆盖/查询、生产 `.tres` 往返与场景启动校验，`2.2` 已让普通 rebuild 与后台 Load preparer 从同一不可变值快照向单 mesh 写入 per-edge width/color，建立六分量 `RoadRenderToken`，并从真实 mesh triangle 同步发布带 matching token 的不可变 `EdgeRibbon` surface。显示 sampler 为每个可见 span 保留源 `GeometryIndex + ParameterStart/ParameterEnd`，普通与 Load triangle 用半开所有权把 geometry join、开放 B 端和 self-loop seam 唯一映射为 canonical `RoadLocation`。degree-1 Node 现按相邻 `RoadTypeStyle.Width / 2` 生成解析 `RoadSurfaceDisc`，以同一宽度/颜色绘制端点 marker，并把 A/B `TerminalCap` 命中固定映射到 canonical 参数 `0/1`；端点高亮也使用相同样式半宽。基础 surface 查询由构造期不可变 AABB 层级统一索引 triangle 与 disc；`RoadSurfaceSnapshot.PreparedData` 在 Load worker 中分别完成两类 primitive 的 defensive copy 与建树，主线程 Preflight 只绑定 reserved token。`v3-save-system:2.3` 及其余 Phase 7 协作项仍开放，因为仍缺 semantic join、junction patch、普通 mutation stalled/retry、类型 UI、RoadUpgrade、缩放/重建视觉矩阵或平行 Edge 工具命中。
>
> 完整自动化当前为 786/786，`RoadSurfaceSnapshotTests` 与 `RoadRendererLoadPrepareTests` 的聚焦组合为 34/34；Debug 与 `ExportRelease` build 均为 0 警告/0 错误，Roslyn compiler/analyzer 与 GDScript workspace scan 为 0 diagnostics。隔离用户目录的 `road_render_token_runtime_contract.gd` 在普通 mutation、样式刷新和 aggregate Load 三条路径均输出 PASS；Godot MCP 看到单条道路 `surfacePrimitiveCount=4`，A/B 命中均为 `TerminalCap`、canonical 参数分别为 `0/1`，且 hit token 与 desired/presented 完全一致。测试槽、动态探针、临时用户目录和日志已清理，editor 无新增错误且 DAP `stderr` 为空。本次 TerminalCap 增量 Vulkan 只执行 1k：camera/preview/highlight P95 为 0.393/0.378/0.339 ms、renderer rebuild 135.918 ms、静态 renderer 节点为 2，并输出 PASS；10k/100k 未重跑，因此第 11 节既有分级 ribbon 规模数据继续作为历史基线，不由本轮 1k 结果替代。renderer lifecycle、闭环普通/Load、RoadType 资源与 mesh 样式契约及 Windows Desktop QA 导出包的既有证据继续保留；更早的统一样式数据、首次冷启动超门和首轮 100k camera 尾延迟仍记录在第 11 节，Phase 8 附录 D 保持为空。
...
| `RoadConfig` 已恰好提供四类合法 `RoadTypeStyle`，普通/Load renderer 以不可变值快照消费 | `RoadTypeStyle.cs`、`RoadConfig.cs`、`RoadRenderer*.cs`、`road_config.tres` | `v3-grid-rendering:2.1`、2.2 分级 ribbon、`EdgeRibbon` + `TerminalCap` surface/index 已验证；semantic join 与 junction patch 仍属 2.2 |
...
| Load 以固定 buffer reader 准备 V3 root，并在同一 aggregate 中交换 graph/tool/basic mesh/ribbon + cap surface/token/slot | `V3JsonStreamReader`、`PreparedAggregateLoad`、`RoadGraph.LoadCommit.cs`、`RoadRenderer.LoadCommit.cs`、`ToolManager.LoadCommit.cs` | 基础 mesh、同源的 indexed `EdgeRibbon` + `TerminalCap` snapshot 与 matching desired/presented token 已原子接管；semantic join 与 junction patch 仍缺 |
...
当前 `v3-save-system:2.3` 仍不能关闭：`PreparedAggregateLoad` 已覆盖 RoadGraph、新的空工具/history、分级基础 `ArrayMesh`/`MultiMesh`、带 canonical `RoadLocation` 和不可变空间索引的同源 `EdgeRibbon` + `TerminalCap` `RoadSurfaceSnapshot`、matching 六分量 desired/presented token 和 `CurrentSlotID`。renderer 的 worker Prepare 已分别完成 triangle/disc defensive copy 与统一索引建树，主线程 Preflight 只绑定 token。现有证据已经覆盖 generation 门禁、提交前失败逐值保留、mesh/ribbon/cap surface/index/token 一次引用交换和 observer warning 隔离，但 renderer 尚未提供 semantic join 或 junction patch owner，也未完成第二 saveable 与每个关键资源故障点的联合矩阵。完整设计仍按下述四阶段收口，不能把当前 ribbon/cap participant 当作最终表现契约。
...
当前 aggregate 已按这一路径交换稳定 RoadGraph facade 的新 lineage、RoadBuilder 的空 placement/removal/history、RoadRenderer 的预建基础 mesh/node batch、同源 triangle/disc 的 indexed `EdgeRibbon` + `TerminalCap` surface、matching token 和槽目标；移除 renderer 的生命周期回归会在 commit 前失败，并逐值保留活动图、未完成 placement、undo/redo 和 `CurrentSlotID`。这项证据只证明已接入的 ribbon/cap participant，不证明尚不存在的 semantic join/junction patch、工具消费或关键资源故障矩阵。
...
6. **部分完成（`v3-save-system:2.3` 开放）**：`SaveOperationCoordinator` 已实现进程内根 gate、结构化 token/state/result、手动优先、pending autosave、取消与退出收敛；公开同步 bool API 已删除。Save/Load/Delete 的长工作移到后台，`PreparedAggregateLoad` 已一次交换 graph、空工具/history、基础 renderer mesh、带 canonical `RoadLocation` 和不可变空间索引的 `EdgeRibbon` + `TerminalCap` surface、matching `RoadRenderToken` 和 slot target；surface triangle/disc 复制与建树在 worker Prepare 完成。尚缺 semantic join、junction patch、第二 saveable 与关键资源全矩阵，不能勾选完成。
...
Phase 5 当前仍为部分完成。`v3-save-system:2.1`～`2.2` 已由存档聚焦组、双配置构建、真实 `MapTest` 和 Windows Desktop QA 导出证明；`2.3` 的 coordinator、结构化状态、后台工作、退出收敛及 basic mesh/`EdgeRibbon` + `TerminalCap` surface/index/token aggregate 已由完整 786/786 自动化、既有运行时契约、隔离用户目录的 `road_render_token_runtime_contract.gd` 和 Godot MCP matching token/location 场景证明。等待 gate 的手动请求会在创建 lease 前重新检查外部取消并释放 gate，场景 drain 不再因取消竞争遗留活动 lease；Load renderer 的 `PreparedData` 也已在 worker 构建统一 AABB 层级，Preflight 不再承担复制或建树。下一入口不再是“创建 coordinator”“建立 render token”“补 ribbon canonical location”“建立基础 surface index”或“生成 terminal cap”，而是与 `v3-grid-rendering:2.2`、`v3-tool-input:2.4`、`v3-ui:1.4` 一起补齐 semantic join、junction patch、工具接管和全部故障矩阵。
...
2. 四类样式资源、per-edge width/color、六分量 desired/presented token，以及同源 `EdgeRibbon` triangle + `TerminalCap` disc 的 `RoadSurfaceSnapshot` / `RoadSurfaceHit` / canonical `RoadLocation` / 不可变空间索引已完成；继续确定 semantic join 与 junction patch owner，再用 fake consumer 验证完整 mesh/surface/token 一次交换。

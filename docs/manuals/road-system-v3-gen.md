...
> 文档状态：实施中；Phase 1～4 与 Phase 6 已完成，Phase 5 的 `v3-save-system:2.1`～`2.2` 已完成、`2.3` 部分实现，Phase 7 的 `v3-grid-rendering:2.1`、分级 ribbon、六分量表现 token、同代 Edge ribbon surface 与 canonical `RoadLocation` 切片已完成且 Load 协作参与者部分接入；当前入口为补齐 surface index 与 junction/cap/join 同源接管
>
> 编写日期：2026-08-14
>
...
> 当前实施记录（2026-08-14）：Phase 1～4 已完成 mutation 数值/容量门禁、endpoint-role incidence、self-loop/parallel Edge、六类原生方向与权威锚、最大连续 Edge、半开 query fragment locality、RoadGroup 移除、closed/self-intersection、四类 `RoadType`、原子 `ChangeRoadType`、不可变 `RoadGraphRevision` root、统一 `GraphChanged`/可逆 delta，以及 lineage/domain revision/change sequence 身份。Phase 5 已完成独立 `user://saves-v3` 根、严格 `simple-cities-v3` format v1、有界 streaming reader、同句柄完整性校验、PNG 预算、五类 occupant、publish/delete descriptor、tombstone、OS 根锁和 digest 恢复矩阵；`SaveOperationCoordinator`、结构化 token/state/result、手动优先与单 pending autosave、取消/退出收敛，以及 RoadGraph + 空工具/history + renderer mesh/Edge ribbon surface + 槽目标的 aggregate Load 已实现，等待 gate 时外部取消不会再产生占锁 lease。Phase 6 的 delta/token 与 entry/估算字节双预算已完成。Phase 7 已部分实现普通与 Load 共用的 closed ribbon、循环 seam join、纯 self-loop seam marker 隐藏，以及两路口环、八字形和删除支路后的 seam 重定位；`v3-grid-rendering:2.1` 已完成四类 `RoadTypeStyle` 资源、严格唯一覆盖/查询、生产 `.tres` 往返与场景启动校验，`2.2` 已让普通 rebuild 与后台 Load preparer 从同一不可变值快照向单 mesh 写入 per-edge width/color，建立六分量 `RoadRenderToken`，并从真实 mesh triangle 同步发布带 matching token 的不可变 `EdgeRibbon` `RoadSurfaceSnapshot`、点命中和矩形 owner 查询。显示 sampler 还为每个可见 span 保留源 `GeometryIndex + ParameterStart/ParameterEnd`，普通与 Load triangle 都用半开所有权把 geometry join、开放 B 端和 self-loop seam 唯一映射为 canonical `RoadLocation`。`v3-save-system:2.3` 及其余 Phase 7 协作项仍开放，因为当前 surface 仍逐 primitive 线性扫描，也仍缺 junction patch/semantic join/terminal cap、普通 mutation stalled/retry、类型 UI、RoadUpgrade、缩放/重建视觉矩阵或平行 Edge 工具命中。完整自动化当前为 779/779；Debug 与 `ExportRelease` build 均为 0 警告/0 错误，Roslyn compiler/analyzer 及修改后的 GDScript 契约为 0 diagnostics。隔离用户目录的 `road_render_token_runtime_contract.gd` 输出 PASS；Godot MCP 已逐步验证普通 mutation、样式刷新和 aggregate Load 的 matching desired/presented/surface hit token 与 `Edge 2 / geometry 0 / parameter 0.5` location。测试槽和临时用户目录已清理，editor 无新增错误且 DAP `stderr` 为空。最新独立真实 Vulkan 运行中，分级 ribbon 的 10k camera/preview/highlight P95 为 0.564/0.827/0.833 ms、Load 与 renderer rebuild 为 665.452 ms；随后 100k 为 8.674/0.977/0.737 ms、重建 4719.153 ms，两档均 PASS 且静态 renderer 节点为 2。renderer lifecycle、闭环普通/Load、RoadType 资源与 mesh 样式契约也均输出 PASS。Windows Desktop QA 导出包继续通过可写与只读 ACL profile。更早的统一样式数据、首次冷启动超门和首轮 100k camera 尾延迟证据仍保留在第 11 节；Phase 8 附录 D 继续保持为空。
...
| `RoadConfig` 已恰好提供四类合法 `RoadTypeStyle`，普通/Load renderer 以不可变值快照消费 | `RoadTypeStyle.cs`、`RoadConfig.cs`、`RoadRenderer*.cs`、`road_config.tres` | `v3-grid-rendering:2.1`、2.2 分级 ribbon 与基础 Edge ribbon surface 已验证；完整 junction/index 仍属 2.2 |
...
当前 token 与基础 surface 已实现：`RoadGraph.FacadeID` 在进程内稳定唯一且 allocator 不回绕，`RoadPresentationTokenTracker` 独立推进六个身份分量。普通 mutation 先推进 desired，只在目标仍匹配时交换完整 mesh/node batch、`RoadSurfaceSnapshot` 并推进 presented；样式刷新单独推进 style revision。Load admission 预留下一次 facade generation/request，Preflight 构造 matching token，aggregate commit 同时交换基础 mesh、带 canonical `RoadLocation` 的 Edge ribbon snapshot 与 desired/presented token，并以 graph state token 消重随后发布的同一次 full reset。这个切片仍没有 surface 空间索引、junction/cap/join owner 或普通 mutation stalled/retry，因此只证明基础 ribbon mesh/surface/token 不混代，不能提前声称 9.4～9.5 的完整命中与接管契约完成。
...
当前 `v3-save-system:2.3` 仍不能关闭：`PreparedAggregateLoad` 已覆盖 RoadGraph、新的空工具/history、分级基础 `ArrayMesh`/`MultiMesh`、带 canonical `RoadLocation` 的同 mesh `EdgeRibbon` `RoadSurfaceSnapshot`、matching 六分量 desired/presented token 和 `CurrentSlotID`。它已经证明 generation 门禁、提交前失败逐值保留、mesh/ribbon surface/token 一次引用交换和 observer warning 隔离，但 renderer 尚未提供 surface 空间索引或 junction/semantic join/terminal cap owner，也未完成第二 saveable 与每个关键资源故障点的联合矩阵。完整设计仍按下述四阶段收口，不能把当前基础 ribbon participant 当作最终表现契约。
...
当前 aggregate 已按这一路径交换稳定 RoadGraph facade 的新 lineage、RoadBuilder 的空 placement/removal/history、RoadRenderer 的预建基础 mesh/node batch、同 triangle 的 Edge ribbon surface、matching token 和槽目标；移除 renderer 的生命周期回归会在 commit 前失败，并逐值保留活动图、未完成 placement、undo/redo 和 `CurrentSlotID`。这项证据只证明已接入的基础 ribbon participant，不证明尚不存在的完整 owner/index、工具消费或关键资源故障矩阵。
...
6. **部分完成（`v3-save-system:2.3` 开放）**：`SaveOperationCoordinator` 已实现进程内根 gate、结构化 token/state/result、手动优先、pending autosave、取消与退出收敛；公开同步 bool API 已删除。Save/Load/Delete 的长工作移到后台，`PreparedAggregateLoad` 已一次交换 graph、空工具/history、基础 renderer mesh、带 canonical `RoadLocation` 的 Edge ribbon surface、matching `RoadRenderToken` 和 slot target。尚缺 surface 空间索引、完整 owner、第二 saveable 与关键资源全矩阵，不能勾选完成。
...
Phase 5 当前仍为部分完成。`v3-save-system:2.1`～`2.2` 已由存档聚焦组、双配置构建、真实 `MapTest` 和 Windows Desktop QA 导出证明；`2.3` 的 coordinator、结构化状态、后台工作、退出收敛及 basic mesh/Edge ribbon surface/token aggregate 已由完整 779/779 自动化、既有运行时契约、隔离用户目录的 `road_render_token_runtime_contract.gd` 和 Godot MCP matching token/location 场景证明。等待 gate 的手动请求会在创建 lease 前重新检查外部取消并释放 gate，场景 drain 不再因取消竞争遗留活动 lease。下一入口不再是“创建 coordinator”“建立 render token”或“补 ribbon canonical location”，而是与 `v3-grid-rendering:2.2`、`v3-tool-input:2.4`、`v3-ui:1.4` 一起把当前基础 surface 扩展为有空间索引和完整 owner 的联合接管，并完成全部故障矩阵。
...
2. 四类样式资源、per-edge width/color、六分量 desired/presented token，以及同 mesh triangle 的 `EdgeRibbon` `RoadSurfaceSnapshot` / `RoadSurfaceHit` / canonical `RoadLocation` 已完成；继续实现 surface 空间索引和确定 junction/cap/join owner，再用 fake consumer 验证完整 mesh/surface/token 一次交换。
...
| `Scripts/Road/RoadSurfaceSnapshot.cs`               | 已实现 Edge ribbon primitive、稳定 owner/hit、canonical location 和线性点/矩形查询；继续增加空间索引与完整 owner |

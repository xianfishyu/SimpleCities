...
| Edge 已强制携带合法 `RoadType`，提交、拆分、交叉、批量改造和归一化均传播类型 | `RoadBuildRequest`、`GraphEdge`、`SubmitPathCore`、`ChangeRoadType` | Phase 4 已完成；`RoadBuilder` 的类型选择/placement 冻结、RoadUpgrade 输入生命周期、RoadType 上下文选择器和 RoadUpgrade 工具呈现均已接入；类型化建造表现门禁仍属 Phase 7 |

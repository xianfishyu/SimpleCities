...
> 文档状态：实施中；Phase 1～4 与 Phase 6 已完成，Phase 5 的 `v3-save-system:2.1`～`2.2` 已完成、`2.3` 部分实现，Phase 7 的 `v3-grid-rendering:2.1`、分级 ribbon、六分量表现 token 与同代 Edge ribbon surface 切片已完成且 Load 协作参与者部分接入；当前入口为补齐 surface index/canonical location 与 junction 同源接管
>
> 编写日期：2026-08-14
>
...
当前 token 基础已实现：`RoadGraph.FacadeID` 在进程内稳定唯一且 allocator 不回绕，`RoadPresentationTokenTracker` 独立推进六个身份分量。普通 mutation 先推进 desired，只在目标仍匹配时交换完整 mesh/node batch 并推进 presented；样式刷新单独推进 style revision。Load admission 预留下一次 facade generation/request，Preflight 构造 matching token，aggregate commit 同时交换基础 mesh 与 desired/presented token，并以 graph state token 消重随后发布的同一次 full reset。这个切片尚未生成 surface snapshot/index，因此当前 token 只证明基础批次不会与图或样式身份混代，不能提前声称 9.4～9.5 的命中与 stalled/retry 契约完成。
...
当前 `v3-save-system:2.3` 仍不能关闭：`PreparedAggregateLoad` 已覆盖 RoadGraph、新的空工具/history、分级基础 `ArrayMesh`/`MultiMesh`、同 mesh triangle 的 `EdgeRibbon` `RoadSurfaceSnapshot`、matching 六分量 desired/presented token 和 `CurrentSlotID`。它已经证明 generation 门禁、提交前失败逐值保留、mesh/ribbon surface/token 一次引用交换和 observer warning 隔离，但 renderer 尚未提供 surface 空间索引、canonical location 或 junction/semantic join/terminal cap owner，也未完成第二 saveable 与每个关键资源故障点的联合矩阵。完整设计仍按下述四阶段收口，不能把当前基础 ribbon participant 当作最终表现契约。
...
6. **部分完成（`v3-save-system:2.3` 开放）**：`SaveOperationCoordinator` 已实现进程内根 gate、结构化 token/state/result、手动优先、pending autosave、取消与退出收敛；公开同步 bool API 已删除。Save/Load/Delete 的长工作移到后台，`PreparedAggregateLoad` 已一次交换 graph、空工具/history、基础 renderer mesh、matching `RoadRenderToken` 和 slot target。尚缺 surface/hit index、第二 saveable 与关键资源全矩阵，不能勾选完成。
...
Phase 5 当前仍为部分完成。`v3-save-system:2.1`～`2.2` 已由存档聚焦组、双配置构建、真实 `MapTest` 和 Windows Desktop QA 导出证明；`2.3` 的 coordinator、结构化状态、后台工作、退出收敛及 basic mesh/token aggregate 已由完整 763/763 自动化、既有运行时契约和 Godot MCP matching token 场景证明。等待 gate 的手动请求会在创建 lease 前重新检查外部取消并释放 gate，场景 drain 不再因取消竞争遗留活动 lease。下一入口不再是“创建 coordinator”或“建立 render token”，而是与 `v3-grid-rendering:2.2`、`v3-tool-input:2.4`、`v3-ui:1.4` 一起把 basic mesh/token aggregate 扩展为完整 surface/hit 联合接管并完成全部故障矩阵。
...
2. 四类样式资源、per-edge width/color、六分量 desired/presented token，以及同 mesh triangle 的 `EdgeRibbon` `RoadSurfaceSnapshot` / `RoadSurfaceHit` 已完成；继续实现 surface 空间索引、canonical location 和确定 junction/cap/join owner，再用 fake consumer 验证完整 mesh/surface/token 一次交换。
...
| `Scripts/Road/RoadSurfaceSnapshot.cs`               | 已实现 Edge ribbon primitive、稳定 owner/hit 和线性点/矩形查询；继续增加 canonical location 与空间索引 |

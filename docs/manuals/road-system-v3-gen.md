...
> 文档状态：实施中；Phase 1～4 与 Phase 6 已完成，Phase 5 的 `v3-save-system:2.1`～`2.2` 已完成、`2.3` 部分实现，Phase 7 的 `v3-grid-rendering:2.1`、分级 ribbon、六分量表现 token、同代 `EdgeRibbon` + `TerminalCap` + `SemanticJoin` surface、canonical `RoadLocation` 与基础 surface 空间索引切片已完成且 Load 协作参与者部分接入；当前入口为补齐 junction patch、普通 mutation stalled/retry 与工具接管
>
> 编写日期：2026-08-14
>
...
> 当前实施记录（2026-08-14）：Phase 1～4 已完成 mutation 数值/容量门禁、endpoint-role incidence、self-loop/parallel Edge、六类原生方向与权威锚、最大连续 Edge、半开 query fragment locality、RoadGroup 移除、closed/self-intersection、四类 `RoadType`、原子 `ChangeRoadType`、不可变 `RoadGraphRevision` root、统一 `GraphChanged`/可逆 delta，以及 lineage/domain revision/change sequence 身份。Phase 5 已完成独立 `user://saves-v3` 根、严格 `simple-cities-v3` format v1、有界 streaming reader、同句柄完整性校验、PNG 预算、五类 occupant、publish/delete descriptor、tombstone、OS 根锁和 digest 恢复矩阵；`SaveOperationCoordinator`、结构化 token/state/result、手动优先与单 pending autosave、取消/退出收敛，以及 RoadGraph + 空工具/history + renderer mesh/ribbon-cap-join surface + 槽目标的 aggregate Load 已实现，等待 gate 时外部取消不会再产生占锁 lease。Phase 6 的 delta/token 与 entry/估算字节双预算已完成。
>
> Phase 7 已部分实现普通与 Load 共用的 closed ribbon、循环 seam join、纯 self-loop seam marker 隐藏，以及两路口环、八字形和删除支路后的 seam 重定位；`v3-grid-rendering:2.1` 已完成四类 `RoadTypeStyle` 资源、严格唯一覆盖/查询、生产 `.tres` 往返与场景启动校验，`2.2` 已让普通 rebuild 与后台 Load preparer 从同一不可变值快照向单 mesh 写入 per-edge width/color，建立六分量 `RoadRenderToken`，并从真实 mesh triangle 同步发布带 matching token 的不可变 `EdgeRibbon` surface。显示 sampler 为每个可见 span 保留源 `GeometryIndex + ParameterStart/ParameterEnd`，普通与 Load triangle 用半开所有权把 geometry join、开放 B 端和 self-loop seam 唯一映射为 canonical `RoadLocation`。degree-1 Node 现按相邻 `RoadTypeStyle.Width / 2` 生成解析 `RoadSurfaceDisc`，以同一宽度/颜色绘制端点 marker，并把 A/B `TerminalCap` 命中固定映射到 canonical 参数 `0/1`；端点高亮也使用相同样式半宽。两条不同 Edge、不同 `RoadType` 的 degree-2 Node 现从实际可见 ribbon 端截面生成同源 `SemanticJoin`：非共线方向把外侧 bevel 缺口按外边中点切成两个纯色三角形，同向方向按固定 RoadType 顺序和最大半宽生成三角 fallback，精确对向方向则由已闭合的 butt 截面直接覆盖而不写入零面积 primitive。方向关系使用 `RoadExactPredicates.Orient2DSign` / `DotSign`，每个 join triangle 携带稳定 Edge/Node/Endpoint、sector order 与固定 canonical endpoint `RoadLocation`，degree-2 semantic boundary 不再绘制圆形 marker。基础 surface 查询由构造期不可变 AABB 层级统一索引 triangle 与 disc；普通 rebuild 与 Load worker 共用 join helper，mesh triangle、surface triangle、颜色、索引、AABB 数据和 token 同代发布。`v3-save-system:2.3` 及其余 Phase 7 协作项仍开放，因为仍缺 junction patch、普通 mutation stalled/retry、类型 UI、RoadUpgrade、缩放/重建视觉矩阵或平行 Edge 工具命中。
>
> 完整自动化当前为 793/793，`RoadSurfaceSnapshotTests` 与 `RoadRendererLoadPrepareTests` 的聚焦组合为 41/41；Debug 与 `ExportRelease` build 均为 0 警告/0 错误，Roslyn compiler/analyzer 与 GDScript workspace scan 为 0 diagnostics。隔离用户目录的 `road_render_token_runtime_contract.gd` 在普通 mutation、样式刷新和 aggregate Load 三条路径均输出 PASS；混合类型 Load 发布 `4 ribbon + 2 TerminalCap + 2 SemanticJoin = 8` 个 primitive，degree-2 boundary marker 已移除，只保留两个远端 cap marker，join 命中返回 matching token、稳定 owner 与 canonical endpoint location。Godot MCP 的单路普通 mutation 仍看到 `surfacePrimitiveCount=4`，ribbon/cap 命中及 matching desired/presented token 正常。测试槽、动态探针、临时用户目录和日志已清理，editor 无新增错误且 DAP `stderr` 为空。本次 SemanticJoin 增量未重跑 10k/100k；此前 TerminalCap 的 1k Vulkan camera/preview/highlight P95 0.393/0.378/0.339 ms、renderer rebuild 135.918 ms 与静态 renderer 节点 2 继续作为历史增量证据，第 11 节既有分级 ribbon 规模数据也不由本轮更新。renderer lifecycle、闭环普通/Load、RoadType 资源与 mesh 样式契约及 Windows Desktop QA 导出包的既有证据继续保留；更早的统一样式数据、首次冷启动超门和首轮 100k camera 尾延迟仍记录在第 11 节，Phase 8 附录 D 保持为空。
...
| `RoadConfig` 已恰好提供四类合法 `RoadTypeStyle`，普通/Load renderer 以不可变值快照消费 | `RoadTypeStyle.cs`、`RoadConfig.cs`、`RoadRenderer*.cs`、`road_config.tres` | `v3-grid-rendering:2.1`、2.2 分级 ribbon、`EdgeRibbon` + `TerminalCap` + `SemanticJoin` surface/index 已验证；junction patch 仍属 2.2 |
...
| Load 以固定 buffer reader 准备 V3 root，并在同一 aggregate 中交换 graph/tool/basic mesh/ribbon + cap + join surface/token/slot | `V3JsonStreamReader`、`PreparedAggregateLoad`、`RoadGraph.LoadCommit.cs`、`RoadRenderer.LoadCommit.cs`、`ToolManager.LoadCommit.cs` | 基础 mesh、同源的 indexed `EdgeRibbon` + `TerminalCap` + `SemanticJoin` snapshot 与 matching desired/presented token 已原子接管；junction patch 仍缺 |
...
当前 token 与基础 surface 已实现：`RoadGraph.FacadeID` 在进程内稳定唯一且 allocator 不回绕，`RoadPresentationTokenTracker` 独立推进六个身份分量。普通 mutation 先推进 desired，只在目标仍匹配时交换完整 mesh/node batch、`RoadSurfaceSnapshot` 并推进 presented；样式刷新单独推进 style revision。Load admission 预留下一次 facade generation/request，worker `RoadRendererLoadPreparer` 先生成分别持有 triangle/disc defensive copy、统一 bounds、节点和 primitive index 的 `RoadSurfaceSnapshot.PreparedData`，Preflight 只构造 matching token 并绑定该 prepared 数据；aggregate commit 同时交换基础 mesh、带 canonical `RoadLocation` 和空间索引的 `EdgeRibbon` + `TerminalCap` + `SemanticJoin` snapshot 与 desired/presented token，并以 graph state token 消重随后发布的同一次 full reset。这个切片仍没有 junction patch 或普通 mutation stalled/retry，因此只证明 ribbon/cap/degree-2 join 的 mesh/surface/index/token 不混代，不能提前声称 9.4～9.5 的完整命中与接管契约完成。
...
当前 `v3-save-system:2.3` 仍不能关闭：`PreparedAggregateLoad` 已覆盖 RoadGraph、新的空工具/history、分级基础 `ArrayMesh`/`MultiMesh`、带 canonical `RoadLocation` 和不可变空间索引的同源 `EdgeRibbon` + `TerminalCap` + `SemanticJoin` `RoadSurfaceSnapshot`、matching 六分量 desired/presented token 和 `CurrentSlotID`。renderer 的 worker Prepare 已分别完成 triangle/disc defensive copy 与统一索引建树，主线程 Preflight 只绑定 token。现有证据已经覆盖 generation 门禁、提交前失败逐值保留、mesh/ribbon/cap/join surface/index/token 一次引用交换和 observer warning 隔离，但 renderer 尚未提供 junction patch owner，也未完成第二 saveable 与每个关键资源故障点的联合矩阵。完整设计仍按下述四阶段收口，不能把当前 ribbon/cap/join participant 当作最终表现契约。
...
当前 aggregate 已按这一路径交换稳定 RoadGraph facade 的新 lineage、RoadBuilder 的空 placement/removal/history、RoadRenderer 的预建基础 mesh/node batch、同源 triangle/disc 的 indexed `EdgeRibbon` + `TerminalCap` + `SemanticJoin` surface、matching token 和槽目标；移除 renderer 的生命周期回归会在 commit 前失败，并逐值保留活动图、未完成 placement、undo/redo 和 `CurrentSlotID`。这项证据只证明已接入的 ribbon/cap/degree-2 join participant，不证明尚不存在的 junction patch、工具消费或关键资源故障矩阵。
...
TerminalCap 增量检查（2026-08-14）只运行 1k Vulkan 数据集：camera/preview/highlight P95 为 0.393/0.378/0.339 ms，renderer rebuild 为 135.918 ms，静态 renderer 节点为 2，契约输出 PASS。它证明在当前小规模门槛中加入 degree-1 terminal 的解析 disc 没有引入静态节点数线性增长或交互帧超限；本轮没有重跑 10k/100k，不能用该结果刷新上面的规模基线，也不能关闭仍要求完整 owner、stalled/retry 和类型改造离散时延的 `v3-grid-rendering:2.3`。
...
6. **部分完成（`v3-save-system:2.3` 开放）**：`SaveOperationCoordinator` 已实现进程内根 gate、结构化 token/state/result、手动优先、pending autosave、取消与退出收敛；公开同步 bool API 已删除。Save/Load/Delete 的长工作移到后台，`PreparedAggregateLoad` 已一次交换 graph、空工具/history、基础 renderer mesh、带 canonical `RoadLocation` 和不可变空间索引的 `EdgeRibbon` + `TerminalCap` + `SemanticJoin` surface、matching `RoadRenderToken` 和 slot target；surface triangle/disc 复制与建树在 worker Prepare 完成。尚缺 junction patch、第二 saveable 与关键资源全矩阵，不能勾选完成。
...
Phase 5 当前仍为部分完成。`v3-save-system:2.1`～`2.2` 已由存档聚焦组、双配置构建、真实 `MapTest` 和 Windows Desktop QA 导出证明；`2.3` 的 coordinator、结构化状态、后台工作、退出收敛及 basic mesh/`EdgeRibbon` + `TerminalCap` + `SemanticJoin` surface/index/token aggregate 已由完整 793/793 自动化、surface/load 聚焦 41/41、既有运行时契约和隔离用户目录的 `road_render_token_runtime_contract.gd` 证明。混合边界 Load 一次发布 8 个 ribbon/cap/join primitive，join hit 的 token、owner 与 endpoint location 保持同代；Godot MCP 另覆盖普通单路 ribbon/cap 场景。等待 gate 的手动请求会在创建 lease 前重新检查外部取消并释放 gate，场景 drain 不再因取消竞争遗留活动 lease；Load renderer 的 `PreparedData` 也已在 worker 构建统一 AABB 层级，Preflight 不再承担复制或建树。下一入口不再是“创建 coordinator”“建立 render token”“补 ribbon canonical location”“建立基础 surface index”“生成 terminal cap”或“生成 semantic join”，而是与 `v3-grid-rendering:2.2`、`v3-tool-input:2.4`、`v3-ui:1.4` 一起补齐 junction patch、工具接管和全部故障矩阵。
...
2. 四类样式资源、per-edge width/color、六分量 desired/presented token，以及同源 `EdgeRibbon` triangle + `TerminalCap` disc + degree-2 `SemanticJoin` triangle 的 `RoadSurfaceSnapshot` / `RoadSurfaceHit` / canonical `RoadLocation` / 不可变空间索引已完成；继续确定 junction patch owner，再用 fake consumer 验证完整 mesh/surface/token 一次交换。
...
| `Scripts/Road/RoadRenderer.cs`                      | 已实现 closed ribbon、per-edge width/color、样式化 terminal cap 与 mesh/surface/token 同代接管；继续负责 semantic join、junction patch 和 stalled/retry |
| `Scripts/Road/RoadSurfaceSnapshot.cs`               | 已实现 `EdgeRibbon` triangle、`TerminalCap` disc、稳定 owner/hit、canonical location、统一不可变 AABB 点/矩形查询及 Load `PreparedData`；继续增加 semantic join 与 junction patch owner |

...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01 通用准备契约与V4-02 存档/工具调用方迁移已完成，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
2026-09-12 已确认[米字网格道路系统规格](../specs/road-system-v4.md)及其两层测试边界，并发布为[GitHub 规格 #1](https://github.com/xianfishyu/SimpleCities/issues/1)。22张工作项为 #2–#23，均作为规格的原生子项，26条原生阻塞关系已核对。[V4-01 #2](https://github.com/xianfishyu/SimpleCities/issues/2)和[V4-02 #3](https://github.com/xianfishyu/SimpleCities/issues/3)已完成，下一项为[V4-03 #4：收拢存档协调器](https://github.com/xianfishyu/SimpleCities/issues/4)。第二票完成网络、工具、表现参与者及场景上下文的通用调用迁移，旧注册入口与V3探针视图暂留；全量964/964、双配置构建及实际V3取消/预检失败隔离/正常加载通过。不代表后续V4行为或性能已通过，性能与资源参数仍按 §11.2 冻结。

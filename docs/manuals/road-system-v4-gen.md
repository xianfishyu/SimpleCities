...
隔离场景新增“选择格段”模式，以青色预选、黄色选择高亮显示实际格段范围；主网格间隔与真实路口截断，普通格心转折及纯环合成seam不额外拆段。按住拖动累积、抬起保留本次选择供验证，Esc清除；选择仅修改会话状态，核心token与codec字节不变。V4-12/13 已新增单格删除与类型改造工具：按住期间只选择当前一格，松开后一次提交；多格批量提交仍由V4-14实现，历史和完整性能仍由后续工单交付。

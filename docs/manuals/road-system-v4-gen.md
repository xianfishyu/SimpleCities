...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01–06 与 V4-16 已完成，隔离场景支持端点续建、转弯及后台操作控制，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
本阶段显式限制为512节点、256边、2048链点；这是有界实现预算，不表示最终路网容量或性能目标已验证。内部交叉、分叉、闭环和完整重叠反馈仍随对应工单扩展。位置参数在一条规范道路边的完整折线上按弧长归一化，不能把它当作端点之间的直线参数。
...
2026-09-12 已确认[米字网格道路系统规格](../specs/road-system-v4.md)及其两层测试边界，并发布为[GitHub 规格 #1](https://github.com/xianfishyu/SimpleCities/issues/1)。22张工作项为 #2–#23，均作为规格的原生子项，26条原生阻塞关系已核对。V4-01–06（#2–#7）与V4-16（#17）已完成，下一项为[V4-07 #8：共线重叠标红并整笔拒绝](https://github.com/xianfishyu/SimpleCities/issues/8)。`Scenes/V4MapTest.tscn` 支持开放端点正反向续建、同类型规范化合并、异类型分界、有限转角连接和schema 3重载。操作控制已加入超过300 ms继续等待、协作取消、清理门禁及目标实际_Draw后的首次frame_post_draw计时；取消与最终发布有明确先后。正式 `MapTest` 仍运行唯一 V3 路网，背景沿用旧版灰色方格。

本轮并行实现最终核心88/88、应用968/968、双配置构建通过；真实Forward+/Vulkan续建与受控异步契约全部通过，旧单道路和空地图回归仍通过。受控等待后的391.7663 ms首次绘制仅证明超时继续处理及准确计时，不是正常性能达标结论。编辑器资源加载和状态检查通过；本次DAP未捕获打印标记，不记为通过。证据见 `.scratch/v4-06-16-qa/verification.md`；评审发现的取消传播遗漏已修复为tool-input:BUG-3。后续交叉、格段编辑、历史、显示失败重试及性能尚未交付；其余参数仍按 §11.2 冻结。

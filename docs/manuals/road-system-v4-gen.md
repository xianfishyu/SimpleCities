...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01–05 已完成存档预重构、隔离空地图和单条独立道路，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
V4-05 在空地图基础上升级为 schema 2，明确拒绝旧 schema 1，不提供迁移器。地图两轴固定为 -4000 至 4000 米，格长只接受 25/50/100/200 米，初始内容版本和两类 ID watermark 均为 1。codec 入口最多读取 4097 字节以识别超限，接受的载荷上限仍为 4096 字节，JSON 深度上限为 4；所有对象拒绝缺失、未知和重复字段。当前只接受空图或两节点、一条独立直线道路。节点字段为 `id/x/y`，道路字段为 `id/startNodeId/endNodeId/profile`；按节点ID稳定排序，reader检查正身份、水位、端点引用、四类profile、主格点坐标与八方向正长度。多道路、连接与交叉格式及其预算随后续切片扩展。
...
2026-09-12 已确认[米字网格道路系统规格](../specs/road-system-v4.md)及其两层测试边界，并发布为[GitHub 规格 #1](https://github.com/xianfishyu/SimpleCities/issues/1)。22张工作项为 #2–#23，均作为规格的原生子项，26条原生阻塞关系已核对。V4-01–05（#2–#6）已完成，下一项为[V4-06 #7：从端点续建与转弯](https://github.com/xianfishyu/SimpleCities/issues/7)。`Scenes/V4MapTest.tscn` 已支持空图上的八方向单笔拖动建造、四类样式、表面来源拾取和schema 2保存重载；第二条道路请求明确拒绝，连接、交叉、合并尚未交付。核心Plan与唯一TryCommit分离，后台纯表现准备和主线程ArrayMesh预检在提交前完成，画面与命中共享转换后的表面及来源版本。正式 `MapTest` 仍运行唯一 V3 路网。

第五票核心48/48、既有965/965、双配置构建通过；真实Forward+/Vulkan覆盖四类型保存重载、八方向输入、零长度和Esc取消，原空地图回归仍通过。编辑器资源重载、实际鼠标建造和版本检查通过；本次DAP未捕获主动打印标记，因此不将空缓冲记为通过。证据见 `.scratch/v4-05-qa/verification.md`；评审发现的极限水位持久化问题已修复并记录为save-system:BUG-17。后续历史、交叉、资源失败重试及性能尚未交付；其余性能与资源参数仍按 §11.2 冻结。
...
| 格式与装配 | V4-05 已落地独立道路 schema 2 和对应表现来源绑定；多道路 payload、完整表现发布与正式切换契约随对应后续切片冻结 |

...
阶段框架采用一条核心主线，读取契约冻结后可并行推进写入与表现准备。所有箭头表示前置依赖，不表示新增写入路径；具体实施切片尚未编排。
...
P0 和 P1 是共同前置。P2 完成后先建立 P3 的领域读取契约；G3 通过后 P4 写入规划与 P6 纯表现数据可以并行。P6 消费冻结的 read snapshot，不通过临时可写 facade 绕过 P3。P3 和 P6 都不能自行创建新的领域写入路径。P4 是核心写入主线，完成后 P5 与 P7 的部分准备可以并行；P7 必须等待 P5 和 P6 的正式接口全部冻结。这里仅澄清已有阶段依赖，正式工作切片仍在设计收口后另行安排。
...
本轮一致性复核已对齐规划/提交与表现预检、P3/P6 契约依赖、性能数据集计数口径及切换前验证/切换后复验的表述。未发现需要新增产品取舍问题的直接矛盾；这只说明文档内部复核结果，不代表实现可行性、运行时或性能已经验证。设计讨论是否收口仍需整体确认，随后再编排实施切片、依赖和验收，正式实现另行确认。

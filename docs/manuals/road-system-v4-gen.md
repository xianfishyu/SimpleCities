...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01–07 与 V4-16 已完成，隔离场景支持端点续建、重叠冲突预览及后台操作控制，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
V4-06 使用 schema 3，拒绝旧 schema 1/2，不提供迁移器。地图两轴固定为 -4000 至 4000 米，格长只接受 25/50/100/200 米，初始内容版本和两类 ID watermark 均为 1。载荷上限为 1 MiB，JSON 深度上限为 6；所有对象拒绝缺失、未知和重复字段。当前接受空图或一条连通、不分叉、不闭合的开放折线，可包含不同道路类型的多条规范道路边。节点字段为 `id/x/y`；道路字段为 `id/startNodeId/endNodeId/profile/points`，points保存包含首尾端点的有序 `x/y` 链点。实体按ID稳定排序，边从较小端点ID指向较大端点ID；reader检查水位、端点、格点、合法方向、必要类型分界及交叉/重叠拒绝。同类型非结构二度连接必须合并，冗余共线点不保留。

本阶段显式限制为512节点、256边、2048链点；这是有界实现预算，不表示最终路网容量或性能目标已验证。内部交叉、分叉和闭环仍随对应工单扩展；共线重叠已统一整笔拒绝，并返回本次草稿的准确冲突区间。位置参数在一条规范道路边的完整折线上按弧长归一化，不能把它当作端点之间的直线参数。
...
2026-09-12 已确认[米字网格道路系统规格](../specs/road-system-v4.md)及其两层测试边界，并发布为[GitHub 规格 #1](https://github.com/xianfishyu/SimpleCities/issues/1)。22张工作项为 #2–#23，均作为规格的原生子项，26条原生阻塞关系已核对。V4-01–07（#2–#8）与V4-16（#17）已完成，下一项为[V4-08 #9：主格点交叉与路口](https://github.com/xianfishyu/SimpleCities/issues/9)。`Scenes/V4MapTest.tscn` 在开放折线续建基础上加入准确重叠预览：后台复用核心PlanBuild判定，只标红草稿冲突跨度，等待或不可建的剩余部分保持中性色；松开独立重新验证并整笔拒绝，不消耗身份或改变已有道路。预览只保留单个worker与最新请求，取消、换图和来源失效均阻止旧结果发布。端点接触不属于重叠，点交叉的路口建设仍待下一票。正式 `MapTest` 继续运行唯一 V3 路网，schema保持3，背景沿用灰色方格。

本票核心112/112、应用968/968、双配置构建通过；真实Forward+/Vulkan覆盖四类型准确红色冲突区段、整笔拒绝、存档字节不变、最新预览合并和旧结果隔离，旧单道路及异步操作回归仍通过。编辑器资源重载和启动正常，MCP交互读取有一次超时，不记为通过；完整交互证据来自独立引擎进程。证据见 `.scratch/v4-07-qa/verification.md`，Standards与Spec评审均无剩余问题。后续路口、格段编辑、历史、显示失败重试及性能尚未交付，其余参数仍按 §11.2 冻结。
...
| 格式与装配 | V4-06 已落地开放折线 schema 3，V4-16 已落地操作和实际绘制门禁；路口/闭环 payload、显示失败恢复与正式切换契约随对应后续切片冻结 |

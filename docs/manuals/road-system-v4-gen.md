...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01–08 与 V4-16 已完成，隔离场景支持主格点路口、重叠预览及后台操作控制，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
V4-08 使用 schema 4，拒绝旧 schema 1/2/3，不提供迁移器。地图两轴固定为 -4000 至 4000 米，格长只接受 25/50/100/200 米，初始内容版本和两类 ID watermark 均为 1。载荷上限为 1 MiB，JSON 深度上限为 6；所有对象拒绝缺失、未知和重复字段。当前接受空图或由主格点路口连接的多个无环路网组件，可包含不同道路类型的多条规范道路边。节点字段为 `id/x/y`；道路字段为 `id/startNodeId/endNodeId/profile/points`，points保存包含首尾端点的有序 `x/y` 链点。实体按ID稳定排序，边从较小端点ID指向较大端点ID；reader检查水位、端点、主格点、合法方向、必要类型分界和共同节点交叉，拒绝未节点化交点与重叠。同类型非结构二度连接必须合并，冗余共线点不保留。

本阶段显式限制为512节点、256边、2048链点；这是有界实现预算，不表示最终路网容量或性能目标已验证。主格点交叉和接入已形成真实路口，格心与闭环仍随后续工单扩展；共线重叠已统一整笔拒绝，并返回本次草稿的准确冲突区间。位置参数在一条规范道路边的完整折线上按弧长归一化，不能把它当作端点之间的直线参数。
...
2026-09-12 已确认规格及其22张工单。V4-01–08（#2–#9）与V4-16（#17）已完成，下一项为[V4-09 #10：格心路口与对角起建](https://github.com/xianfishyu/SimpleCities/issues/10)。隔离V4场景已支持十字、T形、主格点斜向接入及既有路口复用；核心私有切分和规范化后一次发布，路口查询按道路身份和端点角色读取稳定方向与几何转向，表面和拾取共享junction/edge/source归属。schema为4，正式MapTest仍仅运行V3，灰色背景保持。

本票最终核心159/159、应用968/968及双配置构建通过；真实Forward+/Vulkan路口、连通性、表面owner、节点复用、保存重载与重叠拒绝全部通过，旧单道路/后台操作/空图契约仍通过。首次V3零分配断言曾出现64字节偏差，未改相关源码，聚焦和全量复验通过，记录保留。编辑器资源与启动正常，本次DAP未捕获输出，不记为通过。证据见 `.scratch/v4-08-qa/verification.md`，双线评审无剩余问题；格心、闭环、格段编辑、历史和完整性能仍待后续工单。
...
| 格式与装配 | V4-08 已落地主格点路网 schema 4，V4-16 已落地操作和实际绘制门禁；格心/闭环 payload、显示失败恢复与正式切换契约随对应后续切片冻结 |

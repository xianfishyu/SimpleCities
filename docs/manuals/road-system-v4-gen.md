...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01 通用加载准备契约已完成，提交为 `fef39a4`，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
2026-09-12 已确认[米字网格道路系统规格](../specs/road-system-v4.md)及其两层测试边界，并发布为[GitHub 规格 #1](https://github.com/xianfishyu/SimpleCities/issues/1)。22张工作项为 #2–#23，均作为规格的原生子项，26条原生阻塞关系已核对。[V4-01 #2](https://github.com/xianfishyu/SimpleCities/issues/2)已完成并以 `fef39a4` 推送，下一张无前置阻塞工作项为[V4-02 #3](https://github.com/xianfishyu/SimpleCities/issues/3)。首票验证包括全量962/962、Debug/ExportRelease构建及真实V3保存加载和准备失败隔离；不代表后续V4行为或性能已通过，性能与资源参数仍按 §11.2 冻结。

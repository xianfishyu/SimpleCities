...
> 文档状态：重构设计与规格已确认，2026-09-12 更新。规格及22张纵向切片已发布；V4-01–09 与 V4-16 已完成，隔离场景支持主格点与格心路口、重叠预览及后台操作控制，其余切片继续按阻塞关系推进。P0–P9/W0–W9 保留为模块覆盖与依赖参考，不代表 V4 路网已全部实现。
>
...
V4-09 使用 schema 5，拒绝旧 schema 1/2/3/4，不提供迁移器。地图两轴固定为 -4000 至 4000 米，格长只接受 25/50/100/200 米，初始内容版本和两类 ID watermark 均为 1。载荷上限为 1 MiB，JSON 深度上限为 6；所有对象拒绝缺失、未知和重复字段。当前接受空图或由主格点或格心路口连接的多个无环路网组件，可包含不同道路类型的多条规范道路边。节点字段为 `id/x/y`；道路字段为 `id/startNodeId/endNodeId/profile/points`，points保存包含首尾端点的有序 `x/y` 链点。实体按ID稳定排序，边从较小端点ID指向较大端点ID；reader检查水位、端点、合法主格点/格心、格心只沿对角的方向规则、必要类型分界和共同节点交叉，拒绝未节点化交点与重叠。同类型非结构二度连接必须合并，冗余共线点不保留。

本阶段显式限制为512节点、256边、2048链点；这是有界实现预算，不表示最终路网容量或性能目标已验证。主格点与格心交叉已形成真实路口，闭环仍随后续工单扩展；共线重叠已统一整笔拒绝，并返回本次草稿的准确冲突区间。位置参数在一条规范道路边的完整折线上按弧长归一化，不能把它当作端点之间的直线参数。
...
2026-09-12 已确认规格及其22张工单。V4-01–09（#2–#10）与V4-16（#17）已完成。下一阶段可并行推进[V4-10 #11：闭环、自环与不同路径平行边](https://github.com/xianfishyu/SimpleCities/issues/11)和[V4-11 #12：格段预选、拖选与高亮](https://github.com/xianfishyu/SimpleCities/issues/12)。格心按当前格长c/2确定，两轴均半格偏移；Map统一位置和方向合法性，格心仅允许对角道路，重叠分辨率同步半格。schema为5，正式MapTest仍只运行V3。

本票核心216/216、应用968/968及双配置构建通过。真实Forward+/Vulkan四档格心自动交叉、三分支补建、H/V拒绝、节点/端接及表面来源、保存重载和边界封口均通过，旧单道路、后台操作和空图回归通过。编辑器reload无错误，本次交互依据独立引擎进程，不额外声称DAP行为回归通过。证据见 `.scratch/v4-09-qa/verification.md`，双线评审无剩余问题。闭环、格段编辑、历史和完整性能仍待后续票。
...
| 格式与装配 | V4-09 已落地米字网格路网 schema 5，V4-16 已落地操作和实际绘制门禁；闭环 payload、显示失败恢复与正式切换契约随对应后续切片冻结 |

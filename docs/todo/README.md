...
> Source: `.omo/backups/system-doc-split/docs/todo/todolist.md` and `.omo/evidence/split-system-docs/task-3/ownership-map.json`.
> Scope: navigation, legacy source-key mapping, composite notes, integration owners, and overall dependency notes only. This README carries no independent requirements.

## Navigation

- `road-graph`: [docs/todo/road-graph.md](./road-graph.md)
- `persistence`: [docs/todo/persistence.md](./persistence.md)
...
## Composite Notes And Integration Owners

- `todo:item:5.3` is split into `persistence:5.3` and `grid-rendering:5.3`. Integration owner: `persistence`. `persistence:5.3` keeps the checkbox status and save compatibility acceptance. `grid-rendering:5.3` keeps only the visual resource naming side and creates no extra status-bearing checkbox.

## Legacy Source Key Map

| Legacy source key | Kind | Legacy text or ID | Destination refs | Notes |
|---|---|---|---|---|
| `todo:summary:1` | `summary_table_row` | \| 1 \| `AddRoad` 返回 `-1` 时泄漏副作用 \| 已修复 \| 补回归测试，不再修改流程 \| | [road-graph:1](./road-graph.md#road-graph1) |  |
| `todo:summary:2` | `summary_table_row` | \| 2 \| 存档丢失 `RoadType` \| 已修复；v2 命名迁移未完成 \| 补兼容性测试；字段改名延期 \| | [persistence:2](./persistence.md#persistence2) |  |
...
| `todo:summary:11` | `summary_table_row` | \| 11 \| 命名过时且 `RoadType` 视觉样式未落地 \| 事实成立；RoadType 产品功能暂不需要 \| 命名迁移独立处理；分级样式和类型选择延期 \| | [persistence:11](./persistence.md#persistence11) |  |
...
| `todo:summary:RoadType` | `summary_table_row` | \| RoadType \| 数据和存档已完成；视觉、选择与升级当前不需要 \| 基础模型保留；产品功能延期 \| 0.3 只补兼容回归；D5.1～D5.3 与 P6.5 等产品需求确认后启用 \| | [persistence:RoadType](./persistence.md#persistenceroadtype) |  |
| `todo:summary:SaveSystem` | `summary_table_row` | \| SaveSystem \| 当前保存路径、槽名输入和整槽加载事务边界不满足生产存档安全 \| 未完成 \| 0.9～0.11 在 0.1、0.5、0.8 基础上补测试、路径迁移、槽名校验和整槽预检 \| | [persistence:SaveSystem](./persistence.md#persistencesavesystem) |  |
...
| `todo:design-matrix:7a82ab6271cd` | `design_matrix_row` | \| §8 渲染与道路分级 \| 事件驱动渲染和节点绘制已完成；道路分级视觉和类型选择按当前需求延期 \| 0.3；已解决基线；延期 D5.1～D5.3 \| | [grid-rendering:7a82ab6271cd](./grid-rendering.md#grid-rendering7a82ab6271cd) | visual configuration is deferred in grid-rendering |
| `todo:design-matrix:111e2827dafb` | `design_matrix_row` | \| §10 迁移与存档兼容 \| A/B 主体迁移已完成；旧 JSON 字段兼容存在；`SaveManager` 仍写入 `res://saves/<slotName>`、直接插值槽名、顺序恢复已注册系统，缺少版本拒绝、损坏数据保护... | [persistence:111e2827dafb](./persistence.md#persistence111e2827dafb) |  |
...
| `todo:item:0.3` | `todo_checkbox_heading` | 0.3 固化 `RoadType` 存档往返行为（原问题 2） | [persistence:0.3](./persistence.md#persistence0.3) |  |
| `todo:item:0.4` | `todo_checkbox_heading` | 0.4 固化路网与清单的存档版本策略 | [persistence:0.4](./persistence.md#persistence0.4) |  |
| `todo:item:0.5` | `todo_checkbox_heading` | 0.5 为 `RoadGraph` 恢复增加引用校验与失败保护 | [persistence:0.5](./persistence.md#persistence0.5) |  |
...
| `todo:item:0.8` | `todo_checkbox_heading` | 0.8 为 `SaveManager` 增加注销机制并绑定场景生命周期 | [persistence:0.8](./persistence.md#persistence0.8) |  |
| `todo:item:0.9` | `todo_checkbox_heading` | 0.9 建立 `SaveManager` 自动化契约测试 | [persistence:0.9](./persistence.md#persistence0.9) |  |
| `todo:item:0.10` | `todo_checkbox_heading` | 0.10 迁移到 `user://saves` 并限制槽名边界 | [persistence:0.10](./persistence.md#persistence0.10) |  |
| `todo:item:0.11` | `todo_checkbox_heading` | 0.11 建立整槽加载预检与提交边界 | [persistence:0.11](./persistence.md#persistence0.11) |  |
...
| `todo:item:5.3` | `todo_checkbox_heading` | 5.3 独立处理 Junction → Node 命名迁移 | [persistence:5.3](./persistence.md#persistence5.3)<br>[grid-rendering:5.3](./grid-rendering.md#grid-rendering5.3) | legacy 5.3 is split into persistence and grid-rendering entries, with persistence as integration owner Integration owner: `persistence`. |
...
| `todo:item:6.3` | `todo_checkbox_heading` | 6.3 同步活动存档 schema 与迁移策略 | [persistence:6.3](./persistence.md#persistence6.3) |  |
...
| `todo:deferred-section:def6b8230b9f` | `deferred_heading` | Product Phase 6：交通模拟与道路升级 | [traffic-simulation:def6b8230b9f](./traffic-simulation.md#traffic-simulationdef6b8230b9f) |  |
...
| `todo:baseline:878b6f92c0cc` | `solved_baseline_checkbox` | 原问题 2：Edge/Group 的 `RoadType` 已写入并兼容恢复。 `Scripts/Road/RoadGraph.cs:195`、`Scripts/Road/RoadGraph.cs:211`、`Scripts/Road... | [persistence:878b6f92c0cc](./persistence.md#persistence878b6f92c0cc) |  |
...
| `todo:completion:af4fd4e8bade` | `completion_criterion` | 5. 在 Godot 主场景真实完成铺路、交叉、拆除、保存、加载、非法槽名拒绝和加载失败保护验证，并记录实际观察结果；RoadType 产品功能启用后再增加类型选择和样式验证。 | [persistence:af4fd4e8bade](./persistence.md#persistenceaf4fd4e8bade) |  |
...
## Overall Dependency Notes

- `persistence:0.9` depends on `road-graph:0.1` for the automated test entry point.
- `traffic-simulation:P6.1` waits for `road-graph:4.4` to settle post-transaction events or batch change boundaries.
- `traffic-simulation:P6.3` and `traffic-simulation:P6.5` depend on `grid-rendering:D5.1`, `grid-rendering:D5.2`, and `tool-input:D5.3` for RoadType product behavior.
- `grid-rendering:5.3` is linked to `persistence:5.3`; final compatibility acceptance belongs to `persistence`.

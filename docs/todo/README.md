...
> 复核日期：2026-08-13
> 来源：`.omo/backups/system-doc-split/docs/todo/todolist.md`、`.omo/evidence/split-system-docs/task-3/ownership-map.json`、当前根层系统路线图与 `docs/todo/v3/README.md`。
> 范围：仅包含导航、遗留 source key 映射、组合说明、集成负责人和总体依赖说明。本 README 不包含独立要求。

## 导航

> 本索引导航第二代与既有后续系统；第三代道路路线图使用独立索引 [docs/todo/v3/README.md](./v3/README.md)。
...
| `todo:summary:6` | `summary_table_row` | \| 6 \| `RoadGroup` 在合并时丢失用户操作语义 \| 成立 \| 禁止跨 Group/Type 自动合并 \| | [road-graph:6](./road-graph.md#road-graph6) | 遗留原文中的 Type 不再属于 V2；V3 连续 Edge 规范化见 `v3-road-graph:8.2`～`8.5`。 |
...
| `todo:summary:9` | `summary_table_row` | \| 9 \| `GetNeighborIDs().Distinct()` 隐藏平行边 \| 当前为设计选择 \| 保留；明确邻居查询与边查询语义 \| | [road-graph:9](./road-graph.md#road-graph9) | V2 历史决定；V3 的 self-loop/parallel Edge 活动项见 `v3-road-graph:8.1`～`8.3`。 |
| `todo:summary:10` | `summary_table_row` | \| 10 \| 交点判断使用严格浮点相等 \| 成立，低风险 \| 统一改用 epsilon 判断 \| | [road-graph:10](./road-graph.md#road-graph10) |  |
| `todo:summary:11` | `summary_table_row` | \| 11 \| 命名过时且 `RoadType` 视觉样式未落地 \| 事实成立；RoadType 产品功能暂不需要 \| 命名迁移独立处理；分级样式和类型选择延期 \| | [save-system:11](./save-system.md#save-system11) | 遗留原文已取代；V3 分级表现与交互分别见 `v3-grid-rendering`、`v3-tool-input` 和 `v3-ui`。 |
...
| `todo:summary:RoadType` | `summary_table_row` | \| RoadType \| 第二代运行时与存档已移除；视觉、选择与升级当前不需要 \| 第三代产品功能延期 \| V3 活动项在产品需求确认后按独立路线图启用 \| | [save-system:RoadType](./save-system.md#save-systemroadtype) | 遗留原文已失效：V2 已移除 RoadType；V3 活动项见 [独立索引](./v3/README.md)。 |
...
| `todo:summary:Phase 6` | `summary_table_row` | \| Phase 6 \| `TrafficGraph`、A*、拥堵和增量同步 \| 未实现，按路线图延期 \| 第二代 RoadGraph 前置契约已完成；P6.1～P6.4 等待 `v3-road-graph:8.6`，P6.5 已由 V3 道路改造项取代 \| | [traffic-simulation:Phase 6](./traffic-simulation.md#traffic-simulationphase-6) | 遗留原文已取代：P6.1～P6.4 属于 V3 之后的交通模拟；P6.5 已由 V3 道路改造项取代。 |
...
| `todo:design-matrix:7a82ab6271cd` | `design_matrix_row` | \| §8 渲染与道路分级 \| 事件驱动渲染和节点绘制已完成；道路分级视觉和类型选择按当前需求延期 \| 0.3；已解决基线；延期 D5.1～D5.3 \| | [grid-rendering:7a82ab6271cd](./grid-rendering.md#grid-rendering7a82ab6271cd) | 遗留 D5.1～D5.3 已取代；活动项只在 V3 独立路线图中。 |
...
| `todo:design-matrix:95732cb62c0e` | `design_matrix_row` | \| §9、§10 阶段 C、§11 Phase 6 \| `TrafficGraph`、A*、拥堵、增量同步和道路升级工具均未实现 \| 延期 P6.1～P6.5 \| | [traffic-simulation:95732cb62c0e](./traffic-simulation.md#traffic-simulation95732cb62c0e) | 遗留原文已取代：只延期 P6.1～P6.4；P6.5 已迁至 V3 道路改造。 |
...
| `todo:item:0.3` | `todo_checkbox_heading` | 0.3 固化 `RoadType` 存档往返行为（原问题 2） | [save-system:0.3](./save-system.md#save-system0.3) | 遗留标题已取代；当前 V2 结论是移除 RoadType，V3 重新引入由 `v3-save-system:2.1` 跟踪。 |
...
| `todo:item:1.1` | `todo_checkbox_heading` | 1.1 禁止跨 `RoadGroup` 或跨 `RoadType` 自动合并（原问题 6） | [road-graph:1.1](./road-graph.md#road-graph1.1) | 遗留标题中的 RoadType 已退出 V2；该 V2 基线只保留 Group 边界。 |
...
| `todo:deferred:D5.1` | `deferred_checkbox_heading` | D5.1 按产品需求定义 `RoadType` 分级样式 | [grid-rendering:D5.1](./grid-rendering.md#grid-renderingd5.1) | 已取代；活动项为 `v3-grid-rendering:2.1`。 |
| `todo:deferred:D5.2` | `deferred_checkbox_heading` | D5.2 让 `RoadRenderer` 按 `edge.Type` 渲染 | [grid-rendering:D5.2](./grid-rendering.md#grid-renderingd5.2) | 已取代；活动项为 `v3-grid-rendering:2.2`～`2.3`。 |
| `todo:deferred:D5.3` | `deferred_checkbox_heading` | D5.3 让 `RoadBuilder` 提交用户选择的 `RoadType` | [tool-input:D5.3](./tool-input.md#tool-inputd5.3) | 已取代；活动项为 `v3-tool-input:2.1`～`2.2` 与 `v3-ui:1.1`～`1.2`。 |
...
| `todo:deferred:31883cfb1c78` | `deferred_checkbox_heading` | 交通模拟设计时明确平行边策略 | [road-graph:31883cfb1c78](./road-graph.md#road-graph31883cfb1c78) | 已取代；V3 允许平行 Edge，模拟层只定义映射。 |
...
| `todo:deferred:P6.5` | `deferred_checkbox_heading` | P6.5 实现既有道路升级工具 | [traffic-simulation:P6.5](./traffic-simulation.md#traffic-simulationp6.5) | 已取代；活动项为 `v3-road-graph:8.5`、`v3-tool-input:2.2` 和 `v3-ui:1.2`。 |
...
| `todo:baseline:878b6f92c0cc` | `solved_baseline_checkbox` | 原问题 2：Edge/Group 的 `RoadType` 已写入并兼容恢复。 `Scripts/Road/RoadGraph.cs:195`、`Scripts/Road/RoadGraph.cs:211`、`Scripts/Road... | [save-system:878b6f92c0cc](./save-system.md#save-system878b6f92c0cc) | 遗留原文已取代；当前 V2 已移除类型字段和兼容回退。 |
...
| `todo:completion:822fd09c14ca` | `completion_criterion` | 1. 当前清理里程碑要求阶段 0～6 中保留的活动项全部完成，包括 2.4；RoadType 产品功能 D5.1～D5.3、`P6.*` 与其他需求触发项明确排除，直到满足各自启用条件。 | [road-graph:822fd09c14ca](./road-graph.md#road-graph822fd09c14ca) | 遗留 V2 标准；D5.1～D5.3 已由 V3 独立路线图取代，P6.1～P6.4 仍属于 V3 之后。 |
...
| `todo:completion:936efe9cdd8b` | `completion_criterion` | 3. 几何、拓扑、删除事务、存档兼容、`SaveManager` 契约、保存路径边界、整槽加载预检和公共 API 测试全部通过；当前只要求 RoadType 数据/旧存档兼容回归，不要求类型样式或选择 UI。 | [road-graph:936efe9cdd8b](./road-graph.md#road-graph936efe9cdd8b) | 遗留 V2 标准已取代：V2 不再保存 RoadType，也不接受旧类型回退。 |
...
| `todo:completion:af4fd4e8bade` | `completion_criterion` | 5. 在 Godot 主场景真实完成铺路、交叉、拆除、保存、加载、非法槽名拒绝和加载失败保护验证，并记录实际观察结果；RoadType 产品功能启用后再增加类型选择和样式验证。 | [save-system:af4fd4e8bade](./save-system.md#save-systemaf4fd4e8bade) | 遗留 V2 标准；RoadType 选择与样式现由 V3 独立路线图验收。 |
...
- `road-graph:4.4` 已提供事务后稳定事件，作为交通模拟的第二代基线；`traffic-simulation:P6.1`～`P6.4` 仍等待 `v3-road-graph:8.6`。
- `traffic-simulation:P6.3` 还等待 P6.1、P6.2 和独立的模拟参数决策。
- `traffic-simulation:P6.5` 已由 `v3-road-graph:8.5`、`v3-tool-input:2.2` 和 `v3-ui:1.2` 取代；根层 D5.1～D5.3 不再是它的活动依赖。

...
| Geometry                   | Edge 只保存折线 waypoint                    | GraphEdge 已持有直线、Bézier、Hermite 样条与圆弧；其余曲线和新存档未完成 | 2.5～2.6 继续实现一般圆锥曲线、回旋线、V2 存档和权威拓扑运算              |
...
- [ ] **2.4 提供独立于 RoadBuilder 的公共路径提交 API**
  ...
  - 当前进展（2026-08-02）：`SubmitPolyline(IReadOnlyList<Vector2>)` 已提供无 RoadType/网格参数的结构化折线入口，覆盖非法输入与完整覆盖的无副作用拒绝，并让旧 `AddRoad` 复用权威写入链。仍缺 `RoadPath`/`RoadGeometrySegment` 原生曲线请求、未知几何类型、创建实体和完整变更摘要，以及曲线和成功事件一致性验收；依赖 `road-graph:2.5`，本项保持开放。
...
- [ ] **2.5 建立保留真实语义的原生曲线几何模型**
  ...
  - 当前进展（2026-08-03）：新增 `RoadGeometrySegment`、`RoadGeometryKind`、`RoadGeometrySplit`、`LineRoadGeometrySegment`、`CubicBezierRoadGeometrySegment`、`CubicHermiteRoadGeometrySegment` 和 `CircularArcRoadGeometrySegment`，统一使用 `[0, 1]` 参数域并提供位置、单位切线、正向包围盒、长度和开放区间无损拆分契约；权威数据不包含显示采样。cubic Bézier 保留四个控制点，通过导数根计算精确极值包围盒，以弦长/控制多边形上下界递归计算误差受控弧长，并用 De Casteljau 保留拆分后的 cubic 语义。cubic Hermite 样条保留端点和两端导数向量，通过与 cubic Bézier 的解析等价关系复用精确查询，并在拆分后按局部参数区间重标定切向量。圆弧保留圆心、正半径、起始角和不超过一周的有符号扫角，解析计算长度、方向切线、轴向极值包围盒和同类型拆分。`RoadGeometryData` 以显式 `version` 和稳定字符串 `kind` 保存各类型控制参数；`RoadGeometrySerializer` 在创建运行时对象前结构化拒绝无效数据。`GraphEdge.GeometrySegments` 现以只读连续段列表保存权威运行时几何，长度和旧 `Points` 兼容视图均由该列表派生。全部几何聚焦测试 76/76、解决方案测试 161/161、构建 0 警告/0 错误。一般圆锥曲线、回旋线，以及由 `save-system:0.4`、`0.5`、`5.3` 负责的新 V2 原生曲线存档仍未实现，本项保持开放。

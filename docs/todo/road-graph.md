...
| Geometry                   | Edge 只保存折线 waypoint                    | 统一契约和直线段已完成；曲线与 Edge 集成未实现   | 2.5～2.6 继续实现曲线族、序列化和权威拓扑运算                  |
...
- [ ] **2.5 建立保留真实语义的原生曲线几何模型**
  ...
  - 当前进展（2026-08-03）：新增 `RoadGeometrySegment`、`RoadGeometryKind`、`RoadGeometrySplit` 和 `LineRoadGeometrySegment`，统一使用 `[0, 1]` 参数域并提供位置、单位切线、正向包围盒、长度和开放区间无损拆分契约；权威数据不包含显示采样。`RoadGeometrySegmentTests` 14/14 覆盖正反端点、有限性、参数边界、切线、包围盒、拆分连接和长度重组，解决方案测试 99/99、构建 0 警告/0 错误。Bézier、样条、圆弧/圆锥曲线、回旋线、序列化及 GraphEdge 集成尚未实现，本项保持开放。

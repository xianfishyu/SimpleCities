...
| V2 成熟铺路交互 | 连续多段铺路、连续拆除、矩形框选及撤销重做均已完成 | 1.4～1.6 |
...
- [x] **1.5 实现连续拆除和框选删除**
  ...
  - 完成证据（2026-08-04）：新增 `RoadRemovalSession`，普通左键拖动以不大于半个交互半径的间隔查询轨迹并累积排序去重的 Edge ID，`Shift+左键` 则从当前矩形重新生成选择集；选择阶段只更新 `RoadRenderer` 高亮和框线，右键、切出工具或替换输入策略会取消且不写图。`RoadGraph.FindEdgeIDsNear` 使用原生几何圆查询，`FindEdgeIDsIntersecting` 先经空间索引收集候选，再以端点包含和原生几何/矩形边界交点精确过滤；`RemoveEdges` 跳过已失效 ID、去重并按 ID 排序，在一次 `CommitEdgeMutation` 后发布事件。新增 7 项会话测试覆盖空选区、单边、多边、跨 Group、重复轨迹、取消、部分失效目标和曲线 AABB 误选保护；删除/事务聚焦测试 16/16、完整测试 456/456、Debug 构建 0 警告/0 错误。Godot 主场景以真实鼠标事件验证取消、三边连续选择、跨 Group 两边框选、预览清理、存档计数和临时槽清理并输出 PASS；命令中心回归也输出 PASS。`csharp-ls --diagnose` 成功加载解决方案；逐文件 LSP、Godot editor bridge 与 DAP console 因当前通道不可用而阻塞。
...
- [x] **1.6 为铺路和拆路提供撤销与重做**
  ...
  - 完成证据（2026-08-04）：新增容量 64 的 `RoadEditHistory`，以严格 RoadGraph JSON 保存成功事务的前后状态；`RoadBuilder` 的完整路径提交与批量删除共用该边界，失败或无状态变化不入栈，新成功编辑清空重做栈。撤销/重做会先取消未提交会话，再经 `RoadGraph.RestoreState` 恢复 Node/Edge/Group ID、原生几何和 `_nextID`；恢复会重建实体对象，因此 ID 保持而对象引用不保证保持。外部恢复立即清空历史，其他外部图修改在撤销、重做或下一次编辑尝试时使旧历史失效。默认 `Z`/`Y` 动作由 `InputBindingManager` 和 `GameHUD` 路由，`ToolManager` 只委托道路编辑。历史与输入聚焦测试 19/19、完整测试 465/465、Debug 构建 0 警告/0 错误；Godot 主场景验证 5 次成功编辑、拒绝/取消不入栈、两步撤销与重做后的 7/4/2 Edge 渲染和保存状态，命令中心及授权后的暂停菜单契约均输出 PASS。`csharp-ls --diagnose` 成功加载解决方案；逐文件 LSP、Godot editor bridge 与 DAP console 因当前通道不可用而阻塞。
...
- [x] **批量拆除先选择后提交。** `RoadRemovalSession` 只维护稳定 Edge ID 集；连续轨迹和矩形框选共用 `RoadGraph.RemoveEdges` 单次事务，重复或已失效目标不会产生重复副作用。
- [x] **成功道路编辑以完整图状态作为可逆事务边界。** `RoadEditHistory` 保留最多 64 次成功提交；撤销/重做经严格 RoadGraph 恢复同步拓扑、原生几何、渲染和存档，ID 保持但实体对象会重建，失败或外部修改不会污染旧历史。

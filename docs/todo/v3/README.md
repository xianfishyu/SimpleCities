...
- `RoadToolInputRouterTests`：重复命中去重、混合有效/无效命中、全无效不创建会话、无效类型保留 placement/upgrade、同类型保留 placement、矩形空结果、Upgrade/Remove 矩形重复去重、右键移除/清除、切换工具清除 upgrade/removal、非有限坐标抛异常、取消后三类会话不可取回、铺路会话携带当前 RoadType、Cap/JunctionPatch/SemanticJoin owner 命中（Upgrade/Remove）、无 EdgeID owner 拒绝（Upgrade/Remove/矩形/批量）。

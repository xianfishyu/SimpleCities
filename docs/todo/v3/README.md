...
- M4 基础：新增 `V3SaveOperationUiState` 结果映射助手、`V3SaveOperationController` 状态机、`IV3SaveOperationBackend` 后端适配器、`V3SaveSlotUiSummary` 槽展示模型与 `V3SaveOperationUiCoordinator` 统一操作入口及单元测试；`PauseMenu` 已接入 V3 后端（同步包装，待环境运行确认）。

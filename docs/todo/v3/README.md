...
- `V3SaveOperationControllerTests`：开始/重复开始/Cancelling 拒绝开始/完成或 Reset 后可开始、匹配/过期/不同 scene generation/未开始完成、取消后完成仍应用结果、in-progress 保持 token、null 结果抛异常、observer warning 完成、Idle/可取消/已 Cancelling/不可取消阶段取消行为、Reset。

...
| 2.2 | 单一全局样式的开放 ribbon 不能形成可命中的混合宽度完整路面 | 开放（部分实现） | 分级批次、六分量 token、同代四类 surface、canonical `RoadLocation`、基础空间索引、Junction Patch、普通 mutation stalled/retry 与 placement/拆除/改造/历史回放 admission 已建立；继续其余命令接管 |
...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  ...
  - 仍缺（保持开放）：排队 continuation 及其余道路命令尚未接入完整 admission；平行 Edge 仍缺覆盖拆除与改造的完整独立工具矩阵，更新 token 的 renderer 级故障矩阵及每个关键 Load Preflight 故障点也仍需协作验证。
...
- [ ] **2.3 建立混合类型视觉、接管与性能门禁**
  ...
  - 仍缺（保持开放）：Junction Patch 已通过聚焦功能、Load 和普通 T junction 视觉验证，普通 mutation stalled/retry、placement/拆除/RoadUpgrade/undo/redo 命令 admission 也已有功能契约；但其余道路命令的全 admission、离散类型改造时延，以及包含四类 owner 和完整 token 扰动的 10k/100k 性能矩阵仍未完成。现有聚焦证据、四条分级直线截图、ribbon hit、历史 1k cap PASS、patch 小图和小规模 placement/拆除/改造/回放/重试都不能替代完整接管与规模门禁。

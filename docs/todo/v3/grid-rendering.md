...
| 2.2 | 单一全局样式的开放 ribbon 不能形成可命中的混合宽度完整路面 | 开放（部分实现） | 分级批次、六分量 token、同代 Edge ribbon surface、canonical `RoadLocation` 与基础空间索引已建立；继续实现完整 Node surface 与 stalled/retry |
...
| RoadType 与完整道路表面 | 普通 rebuild 与 Load preparer 已从同一不可变 `RoadTypeStyleSnapshot` 为每条 Edge 写入宽度和 vertex color，并从真实 mesh triangle 同步生成带不可变 AABB 索引的 Edge ribbon surface；ribbon hit 已保留源 geometry/参数区间并返回 canonical `RoadLocation`，混合宽度 junction、semantic boundary 和 terminal cap 仍未实现 | 2.1～2.2、`v3-road-graph:8.4`～`8.5` |
| 表现事务与 Load 原子接管 | 普通 mutation 已在完整 mesh/node batch/Edge ribbon surface/index 交换后推进 matching presented token；Load worker 预建 `RoadSurfaceSnapshot.PreparedData`，Preflight 创建 `ArrayMesh`/`MultiMesh` 并只绑定 reserved token，aggregate commit 再交换 snapshot 与 matching token。普通 mutation 尚无 stalled/retry | 2.2、`v3-save-system:2.3`、`v3-tool-input:2.4` |
...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  - 当前问题：`RoadRenderer` 的普通 rebuild 与 Load participant 已按 Edge 消费四类宽度/颜色，并以六分量 desired/presented token 区分 scene、facade、full reset、graph change、样式和 render request；当前还会从每个真实 ribbon mesh triangle 同步生成 `EdgeRibbon` owner 的不可变 `RoadSurfaceSnapshot`。ribbon primitive 已携带源 geometry/参数区间并以半开所有权返回 canonical `RoadLocation`，点/矩形查询也已使用构造期不可变 AABB 层级限制局部候选；但 surface 尚无 junction/semantic join/terminal cap owner。开放 ribbon 加圆形节点仍无法填满混合宽度 T/X/锐角、self-loop 加支路或 degree-2 semantic boundary，容易出现洞、端帽叠层和无限 miter。现有工具尚未消费该 provider。普通 mutation 也尚无可观测 stalled/retry 与道路交互门禁。
  ...
  - Surface 空间索引进展（2026-08-14）：`RoadSurfaceSnapshot` 为 defensive-copy triangle 构建稳定 AABB 层级，叶容量为 8；点/矩形查询先裁剪 node 与 primitive bounds，再执行原有精确 triangle 判定和确定性破同值。查询使用有界 `stackalloc` 遍历栈，不分配候选数组。`RoadSurfaceSnapshot.PreparedData` 不暴露可变数组；普通构造入口继续执行 defensive copy，Load worker 则在 `RoadRendererLoadPreparer.Prepare()` 内完成复制与建树，主线程 Preflight 只将 reserved token 绑定到 prepared 数据。
  ...
  - 仍缺（保持开放）：当前 surface 只有 `EdgeRibbon` triangle；尚无 junction patch、semantic join/terminal cap owner、平行 Edge 工具接线，以及普通 mutation stalled/retry 与道路交互门禁。
...
- [ ] **2.3 建立混合类型视觉、接管与性能门禁**
  ...
  - 仍缺（保持开放）：当前数据尚无 junction patch、完整 surface owner、普通 mutation stalled/retry 或离散类型改造时延；六分量 token 已进入有空间索引的 Edge ribbon surface provider 和 Load hidden snapshot Preflight，但该索引尚未进入 10k/100k surface 性能/扰动矩阵。四条分级直线截图和基础 ribbon hit 不满足本项的混合 junction、完整接管与性能门禁。
...
- [x] **基础 `EdgeRibbon` surface 查询已使用不可变空间索引。** 普通 snapshot 与 Load worker prepared payload 共用同一 AABB 层级；局部点/矩形查询不会随远隔 primitive 总数线性执行精确 triangle 测试，Preflight 不重复复制或建树。完整 owner 和性能矩阵仍由开放的 2.2～2.3 负责。

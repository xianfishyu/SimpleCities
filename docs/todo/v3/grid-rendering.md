...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  ...
  - 拆除 provider 接管证据（2026-08-15）：拆除/history/surface/renderer 聚焦组合 51/51，完整自动化 820/820，双配置 build 均为 0 错误且各有 1 条既有 `NU1900`，Roslyn/GDScript diagnostics 为 0。隔离的 `road_render_token_runtime_contract.gd` 与 `road_input_strategy_runtime_contract.gd` 均 PASS；Godot MCP 验证当前 surface 可选择、样式刷新后旧确认返回 `false` 且 undo 不变、当前 token 删除成功，以及 mutation pending 期间完整旧 edge/cache/mesh/surface 保留、两帧后空图一次发布。测试进程、槽、隔离目录与日志已清理，editor 与 DAP 错误通道为空。
  - 仍缺（保持开放）：RoadUpgrade、类型化建造和排队 continuation 尚未接入 surface/token admission；平行 Edge 仍缺覆盖拆除与改造的完整独立工具矩阵，更新 token 的 renderer 级故障矩阵及每个关键 Load Preflight 故障点也仍需协作验证。

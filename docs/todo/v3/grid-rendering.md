...
| 2.2 | 单一全局样式的开放 ribbon 不能形成可命中的混合宽度完整路面 | 开放（部分实现） | 分级批次、六分量 token、同代四类 surface、canonical `RoadLocation`、基础空间索引、Junction Patch、普通 mutation stalled/retry 与拆除 admission 已建立；继续其余工具/命令接管 |
...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  ...
  - 拆除 provider 接管证据（2026-08-15）：拆除/history/surface/renderer 聚焦组合 51/51，完整自动化 826/826，双配置 build 均为 0 错误且各有 1 条既有 `NU1900`，Roslyn/GDScript diagnostics 为 0。隔离的 `road_render_token_runtime_contract.gd` 与 `road_input_strategy_runtime_contract.gd` 均 PASS；Godot MCP 验证当前 surface 可选择、样式刷新后旧确认返回 `false` 且 undo 不变、当前 token 删除成功，以及 mutation pending 期间完整旧 edge/cache/mesh/surface 保留、两帧后空图一次发布。测试进程、槽、隔离目录与日志已清理，editor 与 DAP 错误通道为空。
  - 类型化建造接入证据（2026-08-15）：`v3-tool-input:2.1` 已完成 `SelectedRoadType`、会话冻结和显式提交；类型聚焦组合 60/60、当前完整自动化 826/826。隔离输入契约验证四类型保存/full-reset Load，Godot MCP 验证非默认类型提交后的 matching 分级 mesh；这项证据不等于 pending/stalled 全命令 admission。
  - 仍缺（保持开放）：RoadUpgrade、类型化建造的 pending/stalled 门禁和排队 continuation 尚未接入 surface/token admission；平行 Edge 仍缺覆盖拆除与改造的完整独立工具矩阵，更新 token 的 renderer 级故障矩阵及每个关键 Load Preflight 故障点也仍需协作验证。
...
- [ ] **2.3 建立混合类型视觉、接管与性能门禁**
  ...
  - 仍缺（保持开放）：Junction Patch 已通过聚焦功能、Load 和普通 T junction 视觉验证，普通 mutation stalled/retry 与拆除命令 admission 也已有功能契约；但 RoadUpgrade/全命令 admission、离散类型改造时延，以及包含四类 owner 和完整 token 扰动的 10k/100k 性能矩阵仍未完成。现有聚焦证据、四条分级直线截图、ribbon hit、历史 1k cap PASS、patch 小图和小规模拆除/重试都不能替代完整接管与规模门禁。
...
- [x] **拆除选择已消费同代道路 surface。** hover、连续轨迹与矩形选择统一查询 frozen `RoadRenderToken` 对应的四类 surface；pending/stalled/superseded 会话清空选择并拒绝确认，提交还校验 graph facade 与 change sequence。RoadUpgrade、全工具矩阵和性能门禁仍由开放的 2.0、2.2～2.3 负责。

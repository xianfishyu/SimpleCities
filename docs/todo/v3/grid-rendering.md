...
| 2.2 | 单一全局样式的开放 ribbon 不能形成可命中的混合宽度完整路面 | 开放（部分实现） | 普通/Load 已共享 per-edge width/color 批次；继续建立完整 surface、hit index 与 token |
...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  - 当前问题：`RoadRenderer` 的普通 rebuild 与基础 Load participant 已按 Edge 消费四类宽度/颜色，但仍只生成逐 Edge ribbon 和统一圆形节点 marker；它不产生 junction/semantic join/terminal cap 的完整 surface owner 或 hit index，且只携带 `GraphStateToken`，样式属性更新也没有 revision/刷新入口。开放 ribbon 加圆形节点无法填满混合宽度 T/X/锐角、self-loop 加支路或 degree-2 semantic boundary，容易出现洞、端帽叠层和无限 miter。工具若仍以中心线 interaction radius 查询，宽路边缘不会稳定进入候选。表现异步接管若只比较 sequence 或分步替换 mesh/index，会产生混代命中；把普通 mutation 的 stalled/retry 模型套到 Load，则会暴露“图已交换但关键表现失败”的非法状态。
  ...
  - 仍缺（保持开放）：`RoadSurfaceSnapshot`、`RoadSurfaceHit`、junction patch、semantic join/terminal cap、surface owner/index、完整六分量 `RoadRenderToken`、`RoadStyleRevision`/属性刷新、普通 mutation stalled/retry 与 Load matching desired/presented token 尚未实现。
...
- [ ] **2.3 建立混合类型视觉、接管与性能门禁**
  ...
  - 仍缺（保持开放）：当前数据尚无 junction patch、surface owner/hit index、六分量 token、普通 mutation stalled/retry、Load hidden surface Preflight、样式 revision 或离散类型改造时延；四条分级直线截图只证明 ribbon width/color，不满足本项的混合 junction、完整接管与性能门禁。

...
| 2.2 | 单一全局样式的开放 ribbon 不能形成可命中的混合宽度完整路面 | 开放（部分实现） | 分级批次、六分量 token、同代四类 surface、canonical `RoadLocation`、基础空间索引、Junction Patch、普通 mutation stalled/retry 与拆除/改造 admission 已建立；继续其余命令接管 |
...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  ...
  - 仍缺（保持开放）：类型化建造的 pending/stalled 门禁和排队 continuation 尚未接入完整 admission；平行 Edge 仍缺覆盖拆除与改造的完整独立工具矩阵，更新 token 的 renderer 级故障矩阵及每个关键 Load Preflight 故障点也仍需协作验证。

...
| 2.2 | 单一全局样式的开放 ribbon 不能形成可命中的混合宽度完整路面 | 开放（部分实现） | 分级批次、六分量 token、同代 Edge ribbon surface 与 canonical `RoadLocation` 已建立；继续实现索引、完整 Node surface 与 stalled/retry |
...
| RoadType 与完整道路表面 | 普通 rebuild 与 Load preparer 已从同一不可变 `RoadTypeStyleSnapshot` 为每条 Edge 写入宽度和 vertex color，并从真实 mesh triangle 同步生成可命中的 Edge ribbon surface；ribbon hit 已保留源 geometry/参数区间并返回 canonical `RoadLocation`，混合宽度 junction、semantic boundary 和 terminal cap 仍未实现 | 2.1～2.2、`v3-road-graph:8.4`～`8.5` |
| 表现事务与 Load 原子接管 | 普通 mutation 已在完整 mesh/node batch/Edge ribbon surface 交换后推进 matching presented token；Load 预留 request，并在 aggregate commit 中交换预建 `ArrayMesh`/`MultiMesh`、`RoadSurfaceSnapshot` 与 matching token。查询仍为线性扫描，没有 surface 空间索引或 stalled/retry | 2.2、`v3-save-system:2.3`、`v3-tool-input:2.4` |
...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  ...
  - 仍缺（保持开放）：当前 surface 只有 `EdgeRibbon` triangle 且逐 primitive 线性扫描；尚无 surface 空间索引、junction patch、semantic join/terminal cap owner、平行 Edge 工具接线，以及普通 mutation stalled/retry 与道路交互门禁。
...
- [ ] **2.3 建立混合类型视觉、接管与性能门禁**
  ...
  - 仍缺（保持开放）：当前数据尚无 junction patch、完整 surface owner/空间索引、普通 mutation stalled/retry 或离散类型改造时延；六分量 token 已进入线性 Edge ribbon surface provider 和 Load hidden snapshot Preflight，但尚未进入 surface 性能/扰动矩阵。四条分级直线截图和基础 ribbon hit 不满足本项的混合 junction、完整接管与性能门禁。

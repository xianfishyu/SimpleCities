...
- [ ] **2.2 在单道路批次中渲染并原子刷新差异化表面**
  ...
  - 仍缺（保持开放）：`RoadSurfaceSnapshot`、`RoadSurfaceHit`、junction patch、per-edge `RoadTypeStyle`、surface owner/index、完整六分量 `RoadRenderToken`、普通 mutation stalled/retry 与 Load matching desired/presented token 尚未实现。
...
- [ ] **2.3 建立混合类型视觉、接管与性能门禁**
  ...
  - 仍缺（保持开放）：当前数据没有 per-edge `RoadTypeStyle`、junction patch、surface owner/hit index、六分量 token、普通 mutation stalled/retry、Load hidden surface Preflight 或离散类型改造时延，因而只作为统一样式对照，不能满足本项的混合类型视觉、接管与性能门禁。

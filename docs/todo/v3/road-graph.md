...
- 空间索引 query fragment 尚未完整接入 revision 诊断；`RoadGraphV3Diagnostics.QueryFragmentCount` 当前按 primitive 计算（line 按 bucket 切分，曲线暂以整体 bounds 作为单个 fragment），`v3-road-graph:8.0`～`8.5` 的完整性能和结构共享门禁仍开放。
...
### 2026-08-13：8.6 当前槽道路网络文件便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotRoadNetworkFile`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽文件大小便捷方法（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetCurrentSlotFileSize(fileName)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽 manifest 有效便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotManifestIsValid`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽道路网络 JSON 长度便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotRoadNetworkJsonLength`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽道路网络 payload 便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotRoadNetworkPayload`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽道路网络 payload 长度便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotRoadNetworkPayloadLength`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽道路网络 payload 存在便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotHasRoadNetworkPayload`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 当前槽道路网络 hash 便捷属性（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `CurrentSlotRoadNetworkHash`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。
...
### 2026-08-13：8.6 任意槽文件 hash 查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotFileHash(slotId, fileName)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽文件大小查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotFileSize(slotId, fileName)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽文件名列表查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotFileNames(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽文件项查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotFile(slotId, fileName)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽 payload 字符串查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotPayloadString(slotId, fileName)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽文件数查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotFileCount(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽总字节查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotTotalBytes(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽城市名查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotCityName(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽 manifest 摘要查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotDisplayName` / `GetSlotTimestamp` / `GetSlotPopulation` / `GetSlotFunds` / `GetSlotThumbnailFile`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽缩略图字节查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotThumbnailBytes(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽 manifest 文件列表查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotManifestFiles(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络 payload 查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkPayload(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络 JSON 查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkJson(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽格式版本查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotFormatFamily` / `GetSlotSchemaVersion`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽 occupant 查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotOccupant(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽可用性查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotIsUsable(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽缩略图 hash 查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotThumbnailHash(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络 hash 查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkHash(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络文件项查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkFile(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络大小查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkSize(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络 payload 存在查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkHasPayload(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络 payload 长度查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkPayloadLength(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络 JSON 长度查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkJsonLength(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽道路网络文件存在查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotRoadNetworkHasFile(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽缩略图文件存在查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotThumbnailHasFile(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽缩略图大小查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotThumbnailSize(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。
...
### 2026-08-13：8.6 任意槽摘要查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotSummary(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 场景根节点清理工具与表现入口（部分）

- `RoadGraphV3System` 新增 `ClearAllToolsAndPresentation`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

### 2026-08-13：8.6 任意槽完整状态查询（部分）

- `RoadGraphV3Application` / `RoadGraphV3System` 新增 `GetSlotIsComplete(slotId)`。
- 完整测试套件 1211/1211 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。
- 尚未完成：替换 V2 RoadSystem 与端到端验收。

...
- 新增 `Scripts/Road/V3/RoadGraphV3System.cs`：Godot `Node2D` 根节点，`_Ready` 时用 `user://saves-v3` 与可选 `RoadConfigV3` 构造 `RoadGraphV3Application`，维护静态 `Instance`，并暴露 `Controller` / `Revision` / `CanUndo` / `CanRedo` / `ClearHistory` / `CurrentSlotID` / `CurrentSlotSummary` / `ToolState` / `TryAddNode` / `TryAddEdge` / `TryBuild` / `TryBuildFromPolyline`（含 snapRadius）/ `TryUpgradeEdges` / `TryChangeRoadType` / `TryRemoveEdge` / `TryRemoveEdges` / `TryUndo` / `TryRedo` / `SaveCurrent` / `SaveAs` / `Load` / `DeleteCurrentSlot` / `GetManifest` / `GetPayload` 便捷属性。

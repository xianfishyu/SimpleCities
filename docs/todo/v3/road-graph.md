...
- 新增 `Scripts/Road/V3/RoadGraphV3System.cs`：Godot `Node2D` 根节点，`_Ready` 时用 `user://saves-v3` 与可选 `RoadConfigV3` 构造 `RoadGraphV3Application`，并暴露 `Controller` / `ToolState` / `TryBuildFromPolyline` 便捷属性。

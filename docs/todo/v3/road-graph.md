...
- 新增 `Scripts/Road/V3/RoadGraphV3System.cs`：Godot `Node2D` 根节点，`_Ready` 时用 `user://saves-v3` 与可选 `RoadConfigV3` 构造 `RoadGraphV3Application`，维护静态 `Instance`，并暴露 `Controller` / `Revision` / `ToolState` / `TryBuildFromPolyline` / `TryUpgradeEdges` / `TryRemoveEdges` / `SaveCurrent` / `Load` / `DeleteCurrentSlot` 便捷属性。
- 已将 `RoadGraphV3System` 节点加入 `Scenes/MapTest.tscn`，并挂载默认 `Scenes/road_config_v3.tres` 与 `RoadGraphV3Renderer` 子节点；Godot 编辑器中场景加载成功，冻结运行无 stderr 错误。

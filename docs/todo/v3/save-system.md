...
- 新增 `Scripts/Core/V3/RoadGraphV3Application.cs`：持有当前 controller、`RoadToolState`、保存根与共享 gate 的 `V3RoadSaveLoadCoordinator` / `V3SlotAutosaveCoordinator` / `V3SlotTransactionCoordinator`，提供 `Save`/`SaveCurrent`/`Load`/`LoadIntoCurrent`/`Delete`/`DeleteCurrentSlot`/`List`/`GetStatus`/`TryAutosave`/`TryAutosaveCurrent`/`NewCity`/`TryUndo`/`TryRedo`/`TryBuild`（含 snapRadius）/`TryUpgrade`/`TryRemove`/`BuildSurfaceSnapshot`/`CurrentTool`/`SelectedRoadType` 同步入口，为真实 Godot RoadSystem/RoadBuilder 装配提供纯 C# 门面。
- 新增 22 个 xUnit 用例；完整测试套件 1033/1033 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。

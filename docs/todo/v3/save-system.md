...
- 新增 `Scripts/Core/V3/RoadGraphV3Application.cs`：持有当前 controller、`Revision`、`RoadToolState`、`DefaultStyles`、`Presentation`、保存根与共享 gate 的 `V3RoadSaveLoadCoordinator` / `V3SlotAutosaveCoordinator` / `V3SlotTransactionCoordinator`，提供 `Save`/`SaveCurrent`/`Load`/`LoadIntoCurrent`/`Delete`/`DeleteCurrentSlot`/`List`/`ListUsableSlots`/`GetStatus`/`TryAutosave`/`TryAutosaveCurrent`/`NewCity`/`TryUndo`/`TryRedo`/`TryBuild`（含 snapRadius）/`TryBuildFromPolyline`/`TryUpgrade`/`TryUpgradeEdges`/`TryRemove`/`TryRemoveEdges`/`BuildSurfaceSnapshot`/`BuildDefaultSurfaceSnapshot`/`TryRequestPresentation`/`CurrentTool`/`SelectedRoadType` 同步入口；构造函数支持注入 `RoadTypeStyleCatalogResult` 或 `RoadConfigV3`。
- 新增 31 个 xUnit 用例；完整测试套件 1042/1042 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。

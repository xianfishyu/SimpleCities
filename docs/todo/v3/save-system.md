...
- 新增 `Scripts/Core/V3/V3SlotTransactionCoordinator.cs`：在进程内 `V3CoordinatorGate` 持有期间统一执行 Publish/Delete/Recover；文件槽存储各自使用根锁保证跨进程排他。
...
- 新增 `Scripts/Core/V3/RoadGraphV3Application.cs`：持有当前 controller、保存根与共享 gate 的 `V3RoadSaveLoadCoordinator` / `V3SlotAutosaveCoordinator`，提供 `Save`/`Load`/`Delete`/`List`/`TryAutosave`/`NewCity`/`TryUndo`/`TryRedo`/`TryBuild`/`TryUpgrade`/`TryRemove`/`BuildSurfaceSnapshot` 同步入口，为真实 Godot RoadSystem/RoadBuilder 装配提供纯 C# 门面。
- 新增 9 个 xUnit 用例；完整测试套件 1010/1010 通过，`dotnet build SimpleCities.sln` 0 警告/0 错误。

...
- [ ] **2.3 建立非阻塞、排他的保存、加载与删除协议**
  ...
  - 仍缺（保持开放）：当前 renderer participant 已覆盖分级 open/closed ribbon、degree-1 terminal cap、degree-2 semantic join、degree≥3 junction patch、节点批次、带 canonical `RoadLocation` 和基础空间索引的同代 surface 与 matching 六分量 token；placement、拆除与 RoadUpgrade 会话及 undo/redo 均已消费并校验 provider token，full reset 保留 `CurrentTool` 与 `SelectedRoadType`。普通 mutation 的 deferred 表现 continuation 会由 generation 阻止旧 callable 跨 commit 消费新请求；未转交的 mesh/node batch 也会在普通失败、Load Preflight 异常或 plan 放弃时确定性释放。aggregate 已分别锁定 observer/cleanup warning 隔离，并以 fake participant 验证 observer/多个 cleanup 同时失败时的 warning 聚合和后续 cleanup；真实 `RoadGraph` participant 的 observer/fake presentation/slot cleanup 组合也已有 CLR 契约，同一场景连续两次 Load 也可再次 admission，fake second saveable 也已服从同一五参与者边界。真实 `RoadGraph`/slot-target commit-boundary generation、真实 `RoadRenderer` admission 和 ToolManager 退树导致的 scene-generation/cancellation 失配已有直接证据。还需协同 `v3-grid-rendering:2.2`、`v3-tool-input:2.4` 完成真实 renderer/tool/slot observer/cleanup 组合、逐关键 Resource 故障及完整故障注入矩阵；若未来引入第二个正式业务 payload，还需为该真实参与者补齐相同矩阵，不能把当前 fake aggregate 或这些切片视为 `2.3` 完成。

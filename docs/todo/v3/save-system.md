...
- 新增 `Scripts/Core/V3/RoadGraphV3Application.cs`：持有当前 controller、保存根与共享 gate 的 `V3RoadSaveLoadCoordinator` / `V3SlotAutosaveCoordinator` / `V3SlotTransactionCoordinator`，提供 `Save`/`Load`/`Delete`/`List`/`TryAutosave`/`NewCity`/`TryUndo`/`TryRedo`/`TryBuild`/`TryUpgrade`/`TryRemove`/`BuildSurfaceSnapshot` 同步入口，为真实 Godot RoadSystem/RoadBuilder 装配提供纯 C# 门面。

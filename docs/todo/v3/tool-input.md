...
| 2.4 | 外部 Load 可能让旧图工具状态或旧画面继续接受输入 | 开放（部分实现） | placement/removal/history 与 matching indexed 四类 surface/token 已加入 full-reset aggregate；补齐 upgrade、surface selection 与命令门禁 |
...
| V3 加载生命周期 | `ToolManager` / `RoadBuilder` 已提供 generation-guarded full-reset plan；成功 aggregate 清空 placement/removal/history，并与基础 mesh、带 canonical `RoadLocation` 和不可变空间索引的同源 `EdgeRibbon` + `TerminalCap` + `SemanticJoin` + `JunctionPatch` surface、matching desired/presented render token 一次交换，失败 Load 保留旧会话。RoadUpgrade、surface hover/selection 和命令门禁尚未存在 | 2.4、`v3-save-system:2.3`、`v3-grid-rendering:2.2`、`v3-road-graph:8.5` |
...
- [ ] **2.4 将成功 full reset 作为旧图工具状态的原子失效边界**
  ...
  - Surface 进展（2026-08-15）：renderer 已在普通 rebuild 和 aggregate Load 中将不可变 `EdgeRibbon` + `TerminalCap` + `SemanticJoin` + `JunctionPatch` `RoadSurfaceSnapshot` 与 mesh/matching token 一次交换，并公开点命中与矩形 owner 查询；ribbon hit 从显示 span provenance 插值得到 canonical `RoadLocation`，degree-1 cap 携带稳定 Edge/Node/Endpoint 并把 A/B 映射到参数 `0/1`，degree-2 join 与 degree≥3 patch triangle 则携带稳定 sector 与固定 canonical endpoint location。Junction Patch 让 self-loop A/B incidence、平行 Edge、T/X/锐角都映射到可执行 Edge owner，并移除 degree≥3 圆形 marker。snapshot 用不可变 AABB 层级统一限制 triangle/disc 局部候选，Load 在 worker `Prepare()` 中完成细分、defensive copy 与索引构建，主线程 Preflight 只绑定 token。普通 mutation 复用同一 preparer，并只重采样失效 Edge；失败保留上一代完整表现、进入带 attempt/异常信息的 stalled，provider 在 presentation 不 current 或 snapshot token 不匹配时拒绝查询，同 token 重试成功后才发布。此切片只建立工具可消费的边界，没有改动 removal 的既有中心线选择，也没有提前接入 upgrade。
  - 当前证据（2026-08-15）：`PreparedAggregateLoadTests`、`RoadJunctionTessellatorTests`、`RoadRendererLoadPrepareTests`、`RoadGeometryDisplaySamplerTests`、`RoadSurfaceSnapshotTests`、history/load/token 契约与完整 817/817 自动化通过。`road_renderer_lifecycle_runtime_contract.gd` 在 renderer participant 缺失时要求 Load 提交前失败，并逐值验证活动 payload、undo/redo、未完成 placement 与 `CurrentSlotID` 不变；成功 V3 Load 已验证创建新 lineage、history 归零且旧 graph token 失效。隔离用户目录的两个道路运行时契约均输出 PASS；混合锐角 Load 发布 `10 ribbon + 2 SemanticJoin + 4 JunctionPatch + 5 cap = 21` 个 primitive、`38 vertices / 5 markers`，并验证 patch owner/location/token；最新 token 契约另验证普通 mutation attempt 1 stalled、attempt 2 同 token 失败、修复后 attempt 3 同 token 成功。`road_closed_ribbon_runtime_contract.gd` 还在已加载图中通过真实 removal 会话选择 seam 侧支路，确认后从 `4 Edge / 38 vertices / 2 markers / 20 primitives` 收敛为 `2 Edge / 21 vertices / 1 marker / 14 primitives`，原 seam 和 junction 不再出现伪 marker。双配置 build 为 0 错误并各有 1 条既有 `NU1900`，Roslyn/GDScript diagnostics、editor 与 DAP 错误通道均通过，测试槽和临时产物已清理；本轮未重跑 10k/100k。
  - 仍缺（保持开放）：尚未实现的 RoadUpgrade/`SelectedRoadType`、surface hover/selection、排队 continuation 和命令级 token admission 无法由当前切片覆盖；也未完成每类工具状态与每个关键 renderer Preflight 故障点的联合矩阵。四类 surface provider、基础空间索引和 provider 级 stalled 门禁已存在，但当前工具尚未消费，不能据此关闭本项。

...
| V3 加载生命周期 | `ToolManager` / `RoadBuilder` 已提供 generation-guarded full-reset plan；成功 aggregate 清空 placement/removal/history，并与基础 mesh、带 canonical `RoadLocation` 的同 mesh Edge ribbon surface、matching desired/presented render token 一次交换，失败 Load 保留旧会话。RoadUpgrade、surface hover/selection 和命令门禁尚未存在 | 2.4、`v3-save-system:2.3`、`v3-grid-rendering:2.2`、`v3-road-graph:8.5` |
...
- [ ] **2.4 将成功 full reset 作为旧图工具状态的原子失效边界**
  ...
  - Surface 进展（2026-08-14）：renderer 已在普通 rebuild 和 aggregate Load 中将不可变 `EdgeRibbon` `RoadSurfaceSnapshot` 与 mesh/matching token 一次交换，并公开点命中与矩形 owner 查询；每个 ribbon hit 现在还可从显示 span provenance 插值得到 canonical `RoadLocation`，geometry join、开放 B 端和 self-loop seam 使用半开唯一所有权。provider 在 presentation 不 current 或 snapshot token 不匹配时拒绝查询。此切片只建立工具可消费的基础边界，没有改动 removal 的既有中心线选择，也没有提前接入 upgrade。
  - 当前证据（2026-08-14）：`PreparedAggregateLoadTests`、`RoadRendererLoadPrepareTests`、`RoadGeometryDisplaySamplerTests`、`RoadSurfaceSnapshotTests`、history/load/token 契约与完整 779/779 自动化通过。`road_renderer_lifecycle_runtime_contract.gd` 在 renderer participant 缺失时要求 Load 提交前失败，并逐值验证活动 payload、undo/redo、未完成 placement 与 `CurrentSlotID` 不变；成功 V3 Load 已验证创建新 lineage、history 归零且旧 graph token 失效。隔离用户目录的 `road_render_token_runtime_contract.gd` 输出 PASS；Godot MCP 进一步验证 Load 在同一次 aggregate 中推进 render facade generation/change sequence，并让 desired/presented/surface hit token 完全匹配，返回 `Edge 2 / geometry 0 / parameter 0.5`；Load 前新增道路的 surface 已失效。`road_closed_ribbon_runtime_contract.gd` 还在已加载图中通过真实 removal 会话选择 seam 侧支路，确认后从 `4 Edge / 20 vertices / 4 markers` 收敛为 `2 Edge / 12 vertices / 2 markers`，原 seam 不再出现伪 marker。
  - 仍缺（保持开放）：尚未实现的 RoadUpgrade/`SelectedRoadType`、完整 surface owner/空间索引、surface hover/selection、排队 continuation 和道路交互门禁无法由当前切片覆盖；也未完成每类工具状态与每个关键 renderer Preflight 故障点的联合矩阵。

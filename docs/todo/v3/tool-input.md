...
| 2.4 | 外部 Load 可能让旧图工具状态或旧画面继续接受输入 | 开放（部分实现） | placement/removal/history 与 matching indexed ribbon/cap surface/token 已加入 full-reset aggregate；补齐 upgrade、surface selection 与命令门禁 |
...
- [ ] **2.4 将成功 full reset 作为旧图工具状态的原子失效边界**
  - 当前问题：基础 aggregate Load 已能原子替换当前已有的 placement、removal、history、renderer overlay、基础 mesh、同源 `EdgeRibbon` triangle + `TerminalCap` disc surface 与 matching 六分量 render token，但 RoadUpgrade、基于已呈现 surface 的 hover/selection、排队道路命令和道路交互门禁尚未实现，因而还不能证明设计列出的全部旧图状态都在同一边界失效。若在解析开始时就取消，损坏、超限或取消的 Load 仍会无故破坏当前会话。
  ...
  - Surface 进展（2026-08-14）：renderer 已在普通 rebuild 和 aggregate Load 中将不可变 `EdgeRibbon` + `TerminalCap` `RoadSurfaceSnapshot` 与 mesh/matching token 一次交换，并公开点命中与矩形 owner 查询；ribbon hit 从显示 span provenance 插值得到 canonical `RoadLocation`，degree-1 cap 则携带稳定 Edge/Node/Endpoint 并把 A/B 映射到参数 `0/1`。snapshot 用不可变 AABB 层级统一限制 triangle/disc 局部候选，Load 在 worker `Prepare()` 中完成分离 defensive copy 与索引构建，主线程 Preflight 只绑定 token。provider 在 presentation 不 current 或 snapshot token 不匹配时拒绝查询。此切片只建立工具可消费的边界，没有改动 removal 的既有中心线选择，也没有提前接入 upgrade。
  - 当前证据（2026-08-14）：`PreparedAggregateLoadTests`、`RoadRendererLoadPrepareTests`、`RoadGeometryDisplaySamplerTests`、`RoadSurfaceSnapshotTests`、history/load/token 契约与完整 786/786 自动化通过；surface/load 聚焦组合为 34/34。`road_renderer_lifecycle_runtime_contract.gd` 在 renderer participant 缺失时要求 Load 提交前失败，并逐值验证活动 payload、undo/redo、未完成 placement 与 `CurrentSlotID` 不变；成功 V3 Load 已验证创建新 lineage、history 归零且旧 graph token 失效。隔离用户目录的 `road_render_token_runtime_contract.gd` 在 mutation/style/Load 路径输出 PASS；Godot MCP 进一步验证 Load 在同一次 aggregate 中推进 render facade generation/change sequence，单路 `surfacePrimitiveCount=4`，A/B `TerminalCap` 命中参数为 `0/1`，且 desired/presented/hit token 完全匹配；Load 前新增道路的 surface 已失效。`road_closed_ribbon_runtime_contract.gd` 还在已加载图中通过真实 removal 会话选择 seam 侧支路，确认后从 `4 Edge / 20 vertices / 4 markers` 收敛为 `2 Edge / 12 vertices / 2 markers`，原 seam 不再出现伪 marker。双配置 build、Roslyn/GDScript diagnostics、editor 与 DAP 错误通道均通过，测试槽和临时产物已清理。
  - 仍缺（保持开放）：尚未实现的 RoadUpgrade/`SelectedRoadType`、semantic join、junction patch、surface hover/selection、排队 continuation 和道路交互门禁无法由当前切片覆盖；也未完成每类工具状态与每个关键 renderer Preflight 故障点的联合矩阵。ribbon/cap provider 与基础空间索引已存在，但当前工具尚未消费，不能据此关闭本项。

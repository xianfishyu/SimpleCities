...
| 2.4 | 外部 Load 可能让旧图工具状态或旧画面继续接受输入 | 开放（部分实现） | placement/removal/upgrade/history、平行 Edge 点选/框选回放与 matching indexed 四类 surface/token 已加入 full-reset aggregate，现有玩家道路事务、deferred 表现 continuation 及未提交表现资源均服从失效/清理边界；真实 `RoadGraph`/slot-target commit-boundary generation、`RoadRenderer` admission、ToolManager scene-generation 下的 placement/removal/RoadUpgrade/hover/bounds 快照、真实 `ToolLoadCommitPlan` 的 commit-boundary generation 原子拒绝，以及真实 renderer observer 分别与 ToolManager/renderer/RoadGraph/slot-target cleanup 的四个同次发生的联合故障子场景已验证，fake participant observer/cleanup 组合和真实 `RoadGraph` observer/fake companion cleanup 组合也有证据，仍需完整真实 `StartLoad()` 故障组合与其余联合故障矩阵 |

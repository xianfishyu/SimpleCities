...
| 2.4 | 外部 Load 可能让旧图工具状态或旧画面继续接受输入 | 开放（部分实现） | placement/removal/history 已加入 full-reset aggregate；补齐 upgrade、surface selection 与完整 token |
...
| V3 加载生命周期 | `ToolManager` / `RoadBuilder` 已提供 generation-guarded full-reset plan；成功 aggregate 清空 placement/removal/history，失败 Load 保留旧会话。RoadUpgrade、surface hover/selection 和 matching render token 尚未存在 | 2.4、`v3-save-system:2.3`、`v3-grid-rendering:2.2`、`v3-road-graph:8.5` |
...
- [ ] **2.4 将成功 full reset 作为旧图工具状态的原子失效边界**
  - 当前问题：基础 aggregate Load 已能原子替换当前已有的 placement、removal、history 和 renderer overlay，但 RoadUpgrade、基于已呈现 surface 的 hover/selection、排队道路命令与完整 render token 尚未实现，因而还不能证明设计列出的全部旧图状态都在同一边界失效。若在解析开始时就取消，损坏、超限或取消的 Load 仍会无故破坏当前会话。
  ...
  - 阶段进展（2026-08-14）：`ToolManager.BeginLoadAdmission()` 捕获当前工具和 generation，`RoadBuilder.PreflightFullReset()` 预建绑定同一 graph facade 的空 `RoadEditHistory`。aggregate commit 清空 placement/removal、鼠标手势标志与旧 history，保留明确的 `CurrentTool`；renderer 同一计划清空 preview、removal preview、selection bounds 和 hover。失败/取消/generation 失配只 dispose replacement，旧状态不变。
  - 当前证据（2026-08-14）：`PreparedAggregateLoadTests`、`RoadRendererLoadPrepareTests`、history/load 契约与完整 727/727 自动化通过。`road_renderer_lifecycle_runtime_contract.gd` 在 renderer participant 缺失时要求 Load 提交前失败，并逐值验证活动 payload、undo/redo、未完成 placement 与 `CurrentSlotID` 不变；成功 V3 Load 已验证创建新 lineage、history 归零且旧 token 失效。`road_closed_ribbon_runtime_contract.gd` 还在已加载图中通过真实 removal 会话选择 seam 侧支路，确认后从 `4 Edge / 20 vertices / 4 markers` 收敛为 `2 Edge / 12 vertices / 2 markers`，原 seam 不再出现伪 marker。
  - 仍缺（保持开放）：尚未实现的 RoadUpgrade/`SelectedRoadType`、surface owner hover/selection、排队 continuation 和六分量 presentation token 无法由当前切片覆盖；也未完成每类工具状态与每个关键 renderer Preflight 故障点的联合矩阵。

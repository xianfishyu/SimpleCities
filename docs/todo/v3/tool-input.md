...
| 2.4 | 外部 Load 可能让旧图工具状态或旧画面继续接受输入 | 开放（部分实现） | placement/removal/history 与 matching render token 已加入 full-reset aggregate；补齐 upgrade、surface selection 与命令门禁 |
...
| V3 加载生命周期 | `ToolManager` / `RoadBuilder` 已提供 generation-guarded full-reset plan；成功 aggregate 清空 placement/removal/history，并与基础 mesh、matching desired/presented render token 一次交换，失败 Load 保留旧会话。RoadUpgrade、surface hover/selection 和命令门禁尚未存在 | 2.4、`v3-save-system:2.3`、`v3-grid-rendering:2.2`、`v3-road-graph:8.5` |
...
- [ ] **2.4 将成功 full reset 作为旧图工具状态的原子失效边界**
  - 当前问题：基础 aggregate Load 已能原子替换当前已有的 placement、removal、history、renderer overlay、基础 mesh 与 matching 六分量 render token，但 RoadUpgrade、基于已呈现 surface 的 hover/selection、排队道路命令和道路交互门禁尚未实现，因而还不能证明设计列出的全部旧图状态都在同一边界失效。若在解析开始时就取消，损坏、超限或取消的 Load 仍会无故破坏当前会话。
  ...
  - 当前证据（2026-08-14）：`PreparedAggregateLoadTests`、`RoadRendererLoadPrepareTests`、history/load/token 契约与完整 763/763 自动化通过。`road_renderer_lifecycle_runtime_contract.gd` 在 renderer participant 缺失时要求 Load 提交前失败，并逐值验证活动 payload、undo/redo、未完成 placement 与 `CurrentSlotID` 不变；成功 V3 Load 已验证创建新 lineage、history 归零且旧 graph token 失效。Godot MCP 进一步验证 Load 在同一次 aggregate 中把 render facade generation/change sequence 各推进一次，desired/presented 完全匹配。`road_closed_ribbon_runtime_contract.gd` 还在已加载图中通过真实 removal 会话选择 seam 侧支路，确认后从 `4 Edge / 20 vertices / 4 markers` 收敛为 `2 Edge / 12 vertices / 2 markers`，原 seam 不再出现伪 marker。
  - 仍缺（保持开放）：尚未实现的 RoadUpgrade/`SelectedRoadType`、surface owner hover/selection、排队 continuation 和道路交互门禁无法由当前切片覆盖；也未完成每类工具状态与每个关键 renderer Preflight 故障点的联合矩阵。

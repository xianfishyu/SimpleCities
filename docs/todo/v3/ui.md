...
| V3 异步存档体验 | PauseMenu 已消费结构化 operation state/result，按 token、menu/scene generation 过滤 continuation，busy 时禁用冲突入口并让 Escape 只请求一次取消；退出流程会 drain/shutdown。Load 已联合发布基础 mesh、`EdgeRibbon` surface 与 matching `RoadRenderToken` acknowledgment，但完整 Node surface/index 尚未存在 | 1.4、`v3-save-system:2.3`、`v3-tool-input:2.4`、`v3-grid-rendering:2.2` |
...
- [ ] **1.4 呈现并约束异步保存、加载与删除操作**
  ...
  - 当前证据（2026-08-14）：`PauseMenuContractTests`、`AutosaveContractTests`、`SaveOperationCoordinatorTests`、`RoadSurfaceSnapshotTests` 与完整 774/774 自动化通过；其中 scene-style drain 回归覆盖等待 gate 的请求与外部取消竞争，不再遗留会阻塞退出的 lease。`pause_menu_runtime_contract.gd`、`autosave_runtime_contract.gd` 和当前 V3 综合运行时契约输出 PASS；Godot MCP 又验证 aggregate Load 联合发布基础 mesh、matching 六分量 `RoadRenderToken` 与同 token 的 `EdgeRibbon` surface，Load 前新增道路的 surface 不再可命中。Debug/`ExportRelease` build 与 Roslyn diagnostics 均为 0，editor 无新增错误且 DAP `stderr` 为空。
  - 仍缺（保持开放）：renderer 的 surface 目前只有线性扫描的 `EdgeRibbon` triangle，没有 canonical location、完整 Node owner 或空间索引；UI 也尚未消费 provider，因此仍不能展示最终的 graph/tool/mesh/surface 一次接管语义。observer/cleanup 每类 warning、所有阶段重复激活与真实关键资源故障矩阵也尚未全部验收。

...
| 1.4 | 暂停菜单没有异步 Save/Load/Delete 的独占状态机 | 开放（部分实现） | operation/render token、generation、busy、Escape、退出收敛、indexed 四类 surface、现有道路事务 admission、deferred continuation 失效、七项 post-commit warning UI、renderer Resource Preflight、Save As 磁盘失败、Overwrite cleanup-pending 及 Delete 失效授权/cleanup-pending 恢复已接入；补齐其余结果矩阵 |
...
- [ ] **1.4 呈现并约束异步保存、加载与删除操作**
  ...
  - 仍缺（保持开放）：renderer 已具备四类 surface、provider 级 stalled/retry 门禁和 deferred continuation 代际失效，placement、拆除、RoadUpgrade 与 undo/redo 也已执行完整 token/graph admission，RoadType 选择器和 RoadUpgrade 双工具入口已有共享状态 UI，PauseMenu 已覆盖 Load 七项 warning 联合显示、干净恢复、一个真实 renderer Resource Preflight 失败格、一个 Save As 磁盘失败恢复格、Overwrite cleanup-pending，以及 Delete 失效授权失败/重试与 cleanup-pending 格；但 Save/Save As 的其余取消与重复激活、Delete 取消/重复激活、Load 各阶段重复激活/取消、逐项 warning 和其余真实关键资源故障矩阵仍未全部验收，因此尚不能关闭最终的 graph/tool/mesh/surface 一次接管语义。

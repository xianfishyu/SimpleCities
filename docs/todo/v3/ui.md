...
| 1.4 | 暂停菜单没有异步 Save/Load/Delete 的独占状态机 | 开放（部分实现） | operation/render token、generation、busy、Escape、退出收敛与基础 indexed ribbon surface 已接入；补齐完整 owner 与结果矩阵 |
...
| V3 异步存档体验 | PauseMenu 已消费结构化 operation state/result，按 token、menu/scene generation 过滤 continuation，busy 时禁用冲突入口并让 Escape 只请求一次取消；退出流程会 drain/shutdown。Load 已联合发布基础 mesh、带 canonical `RoadLocation` 和不可变空间索引的 `EdgeRibbon` surface 与 matching `RoadRenderToken` acknowledgment，但完整 Node surface owner 尚未存在 | 1.4、`v3-save-system:2.3`、`v3-tool-input:2.4`、`v3-grid-rendering:2.2` |
...
- [ ] **1.4 呈现并约束异步保存、加载与删除操作**
  ...
  - 当前证据（2026-08-14）：`PauseMenuContractTests`、`AutosaveContractTests`、`SaveOperationCoordinatorTests`、`RoadGeometryDisplaySamplerTests`、`RoadSurfaceSnapshotTests` 与完整 782/782 自动化通过；其中 scene-style drain 回归覆盖等待 gate 的请求与外部取消竞争，不再遗留会阻塞退出的 lease。surface/load 聚焦组合 27/27 证明 worker prepared 索引可直接绑定 token。`pause_menu_runtime_contract.gd`、`autosave_runtime_contract.gd` 和隔离用户目录的 `road_render_token_runtime_contract.gd` 输出 PASS；Godot MCP 又验证 aggregate Load 联合发布基础 mesh、matching 六分量 `RoadRenderToken` 与同 token 的 indexed `EdgeRibbon` surface，命中返回 `Edge 2 / geometry 0 / parameter 0.5`、`surfacePrimitiveCount=2`，且 Load 前新增道路的 surface 不再可命中。Debug/`ExportRelease` build 与 Roslyn/GDScript diagnostics 均为 0，editor 无新增错误且 DAP `stderr` 为空；测试槽和临时产物已清理。
  - 仍缺（保持开放）：renderer 的 surface 目前只有带基础空间索引的 `EdgeRibbon` triangle，尚无完整 Node owner；UI 也尚未消费 provider，因此仍不能展示最终的 graph/tool/mesh/surface 一次接管语义。observer/cleanup 每类 warning、所有阶段重复激活与真实关键资源故障矩阵也尚未全部验收。

...
`InputBindingManager` 是 `project.godot` 注册的 autoload，管理 11 个单键动作：镜头移动默认 W/A/S/D，选择、铺路、拆路和道路改造默认 Q/R/E/T，道路编辑撤销/重做默认 Z/Y，暂停菜单默认 Esc。点击绑定按钮后，下一次无修饰键的键盘输入会替换该动作；已被其他动作使用的键会被拒绝，不会覆盖两项绑定。再次点击正在监听的按钮或离开按键页会取消监听，“恢复默认”会一次恢复全部 11 项。

每项绑定在成功修改或恢复默认后写入 `user://input_bindings.cfg`。启动时若配置缺失则使用默认值；加载旧配置时先保留已有动作，再为缺失动作补未冲突的默认键，因此旧动作已经使用 T 时不会被新增的 RoadUpgrade 动作覆盖，后者会暂时保持未绑定。配置包含不可用键或重复的已存键时保留整套默认绑定并输出警告。

...
public sealed class RoadRendererLifecycleContractTests
{
    ...
    [Fact]
    public void RealStartLoadRendererAdmissionConstructionFailureRunsAfterReservationBeforePublication()
    {
        string rendererLoadSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Road", "RoadRenderer.LoadCommit.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.True(settingsValidation >= 0 && settingsValidation < renderReservation);
        Assert.True(renderReservation < failure && failure < admissionConstruction);
        Assert.True(admissionConstruction < admissionPublication);
        Assert.Contains(
            "partial void ProbeAggregateLoadRendererAdmissionConstructionFailure();",
            rendererLoadSource,
    ...
    [Fact]
    public void RealStartLoadPostPreparePhaseFailureRunsAfterPhaseAdvanceAndBeforeWorker()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        string failureProbe = ExtractMethod(
            probeSource,
            "partial void ProbeAggregateLoadPostPreparePhaseFailure(",
            "internal void ArmNextAggregateLoadPostPreparePhaseFailure()");

        int participantCaptureFailure = loadOrchestration.IndexOf(
            "ProbeAggregateLoadPostParticipantCaptureFailure(loadParticipants);",
            StringComparison.Ordinal);
        int preparePhase = loadOrchestration.IndexOf(
            "lease.AdvanceTo(SaveOperationPhase.Prepare);",
        ...
        Assert.True(participantCaptureFailure >= 0 &&
            participantCaptureFailure < preparePhase);
        Assert.True(preparePhase < failure && failure < workerStart);
        Assert.True(workerStart < rendererAdmissionDispose);
        Assert.Contains(
    ...
    [Fact]
    public void RealStartLoadWorkerEntryFailureRunsOffMainThreadBeforeSlotPreparation()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.True(workerStart >= 0 && workerStart < workerTimer);
        Assert.True(workerTimer < failure && failure < slotPrepare);
        Assert.True(slotPrepare < rendererWorkerFailure);
        Assert.Contains(
            "partial void ProbeAggregateLoadWorkerEntryFailure();",
            saveManagerSource,
            StringComparison.Ordinal);
    ...
    [Fact]
    public void RealStartLoadPostRendererWorkerPreparationFailureRunsBeforePreparedWorkPublication()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.True(
            rendererWorkerFailure >= 0 && rendererWorkerFailure < presentationPrepare);
        Assert.True(presentationPrepare < failure && failure < preparedWork);
        Assert.True(preparedWork < workerEnd);
        Assert.Contains(
            "partial void ProbeAggregateLoadPostRendererWorkerPreparationFailure(",
            saveManagerSource,
            StringComparison.Ordinal);
    ...
    [Fact]
    public void RealStartLoadPostPreparedWorkReturnFailureRunsOnMainThreadBeforeRequestValidation()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        int workerPreparationFailure = loadOrchestration.IndexOf(
            "ProbeAggregateLoadPostRendererWorkerPreparationFailure(presentation);",
            StringComparison.Ordinal);
        int preparedWork = loadOrchestration.IndexOf(
            "return new PreparedLoadWork(",
        ...
        int failure = loadOrchestration.IndexOf(
            ...
            StringComparison.Ordinal);
        int requestValidation = loadOrchestration.IndexOf(
            "EnsureSceneRequestCurrent(sceneRequest);",
            StringComparison.Ordinal);
        ...
        Assert.True(
            workerPreparationFailure >= 0 && workerPreparationFailure < preparedWork);
        Assert.True(preparedWork < failure && failure < requestValidation);
        Assert.True(requestValidation < preflightPhase);
        Assert.Contains(
            "partial void ProbeAggregateLoadPostPreparedWorkReturnFailure(",
            saveManagerSource,
            StringComparison.Ordinal);
        Assert.Contains(
            "public void ArmAggregateLoadPostPreparedWorkReturnFailure(",
            probeSource,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void RealStartLoadPostSceneRequestValidationFailureRunsOnMainThreadBeforeCancellation()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        int preparedWorkReturnFailure = loadOrchestration.IndexOf(
            "ProbeAggregateLoadPostPreparedWorkReturnFailure(prepared);",
            StringComparison.Ordinal);
        int requestValidation = loadOrchestration.IndexOf(
            "EnsureSceneRequestCurrent(sceneRequest);",
        ...
        Assert.True(
            preparedWorkReturnFailure >= 0 && preparedWorkReturnFailure < requestValidation);
        Assert.True(requestValidation < failure && failure < cancellation);
        Assert.True(cancellation < preflightPhase);
        Assert.Contains(
            "partial void ProbeAggregateLoadPostSceneRequestValidationFailure(",
            saveManagerSource,
            StringComparison.Ordinal);
        Assert.Contains(
            "public void ArmAggregateLoadPostSceneRequestValidationFailure(",
            probeSource,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void RealStartLoadPostPreflightPhaseFailureRunsOnMainThreadBeforeParticipantPreflight()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        int cancellationCheckFailure = loadOrchestration.IndexOf(
            "ProbeAggregateLoadPostCancellationCheckFailure(prepared);",
            StringComparison.Ordinal);
        int preflightPhase = loadOrchestration.IndexOf(
            "lease.AdvanceTo(SaveOperationPhase.Preflight);",
        ...
        Assert.True(preflightPhase < failure && failure < preflightTimer);
        Assert.True(preflightTimer < graphPreflight);
        Assert.Contains(
            "partial void ProbeAggregateLoadPostPreflightPhaseFailure(",
            saveManagerSource,
            StringComparison.Ordinal);
    ...
    [Fact]
    public void RealStartLoadPostToolPreflightFailureTracksTwoPlansBeforeRendererPreflight()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        int graphPreflightFailure = loadOrchestration.IndexOf(
            "ProbeAggregateLoadPostGraphPreflightFailure(",
            StringComparison.Ordinal);
        ...
        Assert.True(toolPlanTracked < failure && failure < rendererPreflight);
        Assert.Contains(
            "partial void ProbeAggregateLoadPostToolPreflightFailure(",
            saveManagerSource,
            StringComparison.Ordinal);
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "preflightPlans[1].ParticipantID",
            failureProbe,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void RealStartLoadPostRendererPreflightFailureTracksThreePlansBeforeSlotPreflight()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        int failure = loadOrchestration.IndexOf(
            ...
            StringComparison.Ordinal);
        int slotPreflight = loadOrchestration.IndexOf(
            "new SlotTargetLoadCommitPlan(",
            StringComparison.Ordinal);

        Assert.True(toolPreflightFailure >= 0 && toolPreflightFailure < rendererPlanTracked);
        Assert.True(rendererPlanTracked < failure && failure < slotPreflight);
        Assert.Contains(
            "partial void ProbeAggregateLoadPostRendererPreflightFailure(",
            saveManagerSource,
            StringComparison.Ordinal);
    ...
    [Fact]
    public void RealStartLoadRendererCommitBoundaryFailureTracksFourPlansBeforeCommit()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.True(graphBoundaryProbe < rendererBoundaryProbe);
        Assert.True(rendererBoundaryProbe < toolBoundaryProbe && toolBoundaryProbe < aggregateCommit);
        Assert.Contains(
            "partial void ProbeAggregateLoadRendererCommitBoundaryGenerationMismatch(",
            saveManagerSource,
            StringComparison.Ordinal);
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "new AggregateLoadRendererBoundaryInvalidatingLease(",
            failureProbe,
            StringComparison.Ordinal);
        Assert.Contains("operationLease = wrapper;", failureProbe, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void RealStartLoadToolCommitBoundaryFailureTracksFourPlansBeforeCommit()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.True(rendererBoundaryProbe < toolBoundaryProbe);
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "new AggregateLoadToolBoundaryInvalidatingLease(",
            failureProbe,
            StringComparison.Ordinal);
        Assert.Contains("operationLease = wrapper;", failureProbe, StringComparison.Ordinal);
    }
    ...
    public void RealStartLoadRoadMeshFactoryFailureObservesTwoPlansBeforeRendererPreflight()
    {
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "currentRenderer.ArmNextAggregateLoadRoadMeshFactoryFailure()",
            observationArm,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            ...
            observationProbe,
            StringComparison.Ordinal);
        Assert.Contains("state.Armed = false;", observationCapture, StringComparison.Ordinal);
        Assert.Contains("state.Count++;", observationCapture, StringComparison.Ordinal);
        Assert.Contains(
            "System.Environment.CurrentManagedThreadId == _mainThreadID,",
            observationCapture,
            StringComparison.Ordinal);
        Assert.Contains("(int)phase,", observationCapture, StringComparison.Ordinal);
        Assert.Contains("preflightPlans.Count,", observationCapture, StringComparison.Ordinal);
        Assert.Contains("preflightPlans[0].ParticipantID", observationCapture, StringComparison.Ordinal);
        Assert.Contains("preflightPlans[1].ParticipantID", observationCapture, StringComparison.Ordinal);
        Assert.Contains("targetRevision.Edges.Count,", observationCapture, StringComparison.Ordinal);
    ...
    [Fact]
    public void RealStartLoadNodeBatchFactoryFailureSharesStructuredPreflightObservation()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "currentRenderer.ArmNextAggregateLoadNodeBatchFactoryFailure()",
            observationArm,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void RealStartLoadRoadSurfaceSnapshotFailureSharesStructuredRendererPreflightObservation()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        Assert.Contains(
            ...
            StringComparison.Ordinal);
        Assert.Contains(
            "currentRenderer.ArmNextAggregateLoadRoadSurfaceSnapshotFailure()",
            observationArm,
            StringComparison.Ordinal);
        Assert.Contains(
    ...
    [Fact]
    public void RealStartLoadRendererCommitBoundaryProbeInvalidatesInsideTheStorageBoundary()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        string invalidatingLease = ExtractMethod(
            ...
            "private sealed class AggregateLoadRendererBoundaryInvalidatingLease(",
            "public partial class ToolManager");
        string rendererInvalidation = ExtractMethod(
            probeSource,
        ...
        int operationLeaseCapture = loadOrchestration.IndexOf(
            "IStorageOperationLease aggregateOperationLease = lease;",
            StringComparison.Ordinal);
        int boundaryProbe = loadOrchestration.IndexOf(
            "ProbeAggregateLoadRendererCommitBoundaryGenerationMismatch(",
            StringComparison.Ordinal);
        int aggregateCommit = loadOrchestration.IndexOf(
            "aggregate.Commit(aggregateOperationLease)",
        ...
        Assert.True(ownershipTransfer >= 0 && ownershipTransfer < operationLeaseCapture);
        Assert.True(operationLeaseCapture < boundaryProbe && boundaryProbe < aggregateCommit);
        Assert.True(wrapperCreation >= 0);
        Assert.Contains("operationLease = wrapper;", failureProbe, StringComparison.Ordinal);
        Assert.True(boundaryEntry >= 0 && boundaryEntry < admissionInvalidation);
        Assert.True(admissionInvalidation < aggregateBoundaryAction);
        ...
        Assert.Contains("inner.AcquireCommitLease();", invalidatingLease);
        Assert.Contains("inner.MarkCommitted();", invalidatingLease);
        Assert.Contains("RoadRendererLoadAdmission admission = _loadAdmission ??", rendererInvalidation);
        Assert.Contains("admission.Dispose();", rendererInvalidation);
        Assert.Contains(
            "partial void ProbeAggregateLoadRendererCommitBoundaryGenerationMismatch(",
            saveManagerSource,
            StringComparison.Ordinal);
    ...
    [Fact]
    public void RealStartLoadToolCommitBoundaryProbeInvalidatesInsideTheStorageBoundary()
    {
        string saveManagerSource = File.ReadAllText(
            Path.Combine(ProjectRoot, "Scripts", "Core", "SaveManager.cs"));
        string probeSource = File.ReadAllText(
        ...
        string invalidatingLease = ExtractMethod(
            ...
            "public partial class ToolManager");
        string toolInvalidation = ExtractMethod(
            probeSource,
        ...
        Assert.True(ownershipTransfer >= 0 && ownershipTransfer < operationLeaseCapture);
        Assert.True(operationLeaseCapture < rendererBoundaryProbe);
        Assert.True(rendererBoundaryProbe < toolBoundaryProbe && toolBoundaryProbe < aggregateCommit);
        Assert.True(wrapperCreation >= 0);
        Assert.Contains("operationLease = wrapper;", failureProbe, StringComparison.Ordinal);
        Assert.True(boundaryEntry >= 0 && boundaryEntry < admissionInvalidation);
        ...
        Assert.Contains("inner.AcquireCommitLease();", invalidatingLease);
        Assert.Contains("inner.MarkCommitted();", invalidatingLease);
        Assert.Contains("ToolLoadAdmission admission = _loadAdmission ??", toolInvalidation);

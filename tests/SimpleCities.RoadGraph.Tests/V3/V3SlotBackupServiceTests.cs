...
public sealed class V3SlotBackupServiceTests
{
    [Fact]
    public void Backup_ThenRestore_RoundTrips()
    {
        ...
        try
        {
            ...
            Assert.True(V3SlotBackupService.Backup("city-001", root, backupRoot));
            Assert.True(store.Delete("city-001"));
            Assert.True(V3SlotBackupService.Restore("city-001", root, backupRoot));
    ...
    [Fact]
    public void Backup_MissingSlot_Fails()
    {
        string root = GetTempRoot();
        string backupRoot = GetTempRoot();
        try
        {
            Assert.False(V3SlotBackupService.Backup("missing", root, backupRoot));
        }
        finally
        {
            Cleanup(root);
    ...
    [Fact]
    public void Restore_MissingBackup_Fails()
    {
        string root = GetTempRoot();
        string backupRoot = GetTempRoot();
        try
        {
            Assert.False(V3SlotBackupService.Restore("missing", root, backupRoot));
        }
        finally
        {
            Cleanup(root);
    ...
    private static string GetTempRoot() =>
        Path.Combine(Path.GetTempPath(), $"v3-backup-{Guid.NewGuid():N}");

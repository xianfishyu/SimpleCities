...
public sealed class V3SlotMoveServiceTests
{
    [Fact]
    public void Move_MovesSlotToDestination()
    {
        ...
        try
        {
            ...
            Assert.True(V3SlotMoveService.Move("city-001", sourceRoot, destinationRoot));
    ...
    [Fact]
    public void Move_MissingSlot_Fails()
    {
        string sourceRoot = GetTempRoot();
        string destinationRoot = GetTempRoot();
        try
        {
            Assert.False(V3SlotMoveService.Move("missing", sourceRoot, destinationRoot));
        }
        finally
        {
            Cleanup(sourceRoot);
    ...
    [Fact]
    public void Move_CorruptSlot_FailsAndKeepsSource()
    {
        string sourceRoot = GetTempRoot();
        string destinationRoot = GetTempRoot();
        try
        {
            ...
            Assert.False(V3SlotMoveService.Move("city-001", sourceRoot, destinationRoot));
    ...
    private static string GetTempRoot() =>
        Path.Combine(Path.GetTempPath(), $"v3-move-{Guid.NewGuid():N}");

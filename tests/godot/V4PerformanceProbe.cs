    ...
    public async void PreparePerformanceFixture(int cell)
    {
        ...
        try
        {
            ...
            _performanceFixture = new()
            {
                ...
                ["polylineEdges"] = snapshot.Edges.Count(edge => edge.Points.Count > 2),
                ["maximumPointsPerEdge"] = snapshot.Edges.Max(edge => edge.Points.Count),
                ["junctions"] = snapshot.Nodes.Count(node =>
                    RoadJunctionQuery.Read(snapshot, node.Id) is { Incidences.Count: >= 3 }),
                ...
                ["setup"] = "public planner plus strict codec roundtrip; setup excluded from sampled windows",
            };
        }

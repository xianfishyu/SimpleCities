...
func run() -> void:
	...
	var prepare_recovered: Dictionary = await require_update_token_prepare_failure(
		...
		builder,
		recovered)
	if prepare_recovered.is_empty():
		return
	recovered = prepare_recovered
	...
	var precommit_recovered: Dictionary = \
		await require_update_token_precommit_supersession(renderer, builder, recovered)
	if precommit_recovered.is_empty():
		return
	recovered = precommit_recovered
	var precommit_style_recovered: Dictionary = \
		await require_update_token_precommit_road_style_supersession(
			renderer,
			builder,
			recovered)
	if precommit_style_recovered.is_empty():
		return
	recovered = precommit_style_recovered
	...
	if not require_junction_patch_hit(
		...
		"Consecutive aggregate Load"):
		return
	var precommit_scene_recovered: Dictionary = \
		await require_update_token_precommit_scene_generation_supersession(
			renderer,
			builder,
			consecutively_loaded)
	...
	if precommit_graph_generation_recovered.is_empty():
		return
	var precommit_change_sequence_recovered: Dictionary = \
		await require_update_token_precommit_change_sequence_supersession(
			renderer,
			builder,
...
func require_update_token_prepare_failure(
	...
	if not require(probe_script != null, "Debug prepare failure probe did not load"):
		return {}
	var probe: RefCounted = probe_script.new()
	if not require(probe != null, "Debug prepare failure probe did not instantiate"):
		return {}
	...
	var failure_message := str(probe.GetPrepareFailureMessage())
	probe.ArmPrepareFailure(renderer)
	if not require(
		bool(probe.IsPrepareFailureArmed()),
		"Update-token prepare failure probe did not arm"):
		return {}

	if not require(
		builder.BeginPlace(Vector2(0.0, 1700.0)),
		"Prepare failure mutation did not begin"):
		return {}
	builder.UpdatePlace(Vector2(100.0, 1700.0))
	if not require(
		builder.CommitPlace(Vector2(100.0, 1700.0)),
		"Prepare failure mutation did not commit"):
		return {}
	await process_frame
	await process_frame
	...
	if not require(
		...
		"Prepare failure did not stall the new desired token"):
		return {}
	if not require_ordinary_change(before, desired):
		return {}
	if not require(
		...
		int(probe.GetPrepareFailureCount()) == 1 and
		not bool(probe.IsPrepareFailureArmed()),
		"Prepare failure was not reported as the one-shot first attempt"):
		return {}
	if not require(
		...
		"Prepare failure allocated resources or replaced retained presentation"):
		return {}
	if not require(
		renderer.FindRoadSurfaceHit(Vector2(50.0, 0.0), 0.0).is_empty() and
		renderer.FindRoadSurfaceHit(Vector2(50.0, 1700.0), 0.0).is_empty(),
		"Prepare failure exposed old or unpresented surface data"):
		return {}

	if not require(
		renderer.RetryRoadPresentation(),
		"Prepare failure retry did not publish the same desired token"):
		return {}
	var recovered := presentation_token(renderer, "Prepare failure retry")
	if recovered.is_empty():
		return {}
	if not require(
		...
		renderer.GetRoadMeshVertexCount() > retained_vertex_count and
		renderer.GetNodeMarkerCount() > retained_marker_count,
		"Prepare failure retry did not atomically publish attempt two"):
		return {}
	if not require_surface_hit(
		renderer,
		Vector2(50.0, 1700.0),
		recovered,
		"Prepare failure retry"):
		return {}
	print(
		(
			"UPDATE_TOKEN_PREPARE_FAILURE_RESULT resource_before=%d resource_after=%d " +
			"trigger_count=%d stalled_attempt=1 recovered_attempt=2"
		) % [
			resource_count_before,
			int(probe.GetObjectResourceCount()),
			int(probe.GetPrepareFailureCount()),
		])
	return recovered
...
func require_update_token_precommit_supersession(
	renderer: Node,
	builder: Node,
	before: Dictionary
) -> Dictionary:
	var probe_script: Script = load(UPDATE_TOKEN_FAILURE_PROBE_PATH)
	if not require(probe_script != null, "Debug pre-commit supersession probe did not load"):
		return {}
	var probe: RefCounted = probe_script.new()
	if not require(probe != null, "Debug pre-commit supersession probe did not instantiate"):
		return {}
	...
	if not require(
		...
		"Update-token pre-commit supersession probe did not arm"):
		return {}

	if not require(
		builder.BeginPlace(Vector2(0.0, 900.0)),
		"Pre-commit supersession mutation did not begin"):
		return {}
	builder.UpdatePlace(Vector2(100.0, 900.0))
	if not require(
		builder.CommitPlace(Vector2(100.0, 900.0)),
		"Pre-commit supersession mutation did not commit"):
		return {}
	await process_frame
	await process_frame
	...
	if not require(
		...
		"Pre-commit supersession did not retain the old presentation as pending"):
		return {}
	...
	if not require(
		require_same(superseded, replacement, [
			...
			"changeSequence",
			"roadStyleRevision",
		], "Pre-commit supersession") and
		int(replacement.renderRequestID) == int(superseded.renderRequestID) + 1,
		"Pre-commit supersession did not advance only RenderRequestID"):
		return {}
	if not require(
		int(pending.get("attemptCount", -1)) == 0 and
		int(probe.GetPreCommitTokenSupersessionCount()) == 1 and
		int(probe.GetPreCommitSupersededAttemptNumber()) == 1 and
		not bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Pre-commit supersession was not raised inside the first target attempt"):
		return {}
	if not require(
		int(probe.GetObjectResourceCount()) == resource_count_before and
		...
		renderer.GetRoadMeshVertexCount() == retained_vertex_count and
		renderer.GetNodeMarkerCount() == retained_marker_count,
		"Pre-commit supersession leaked resources or replaced retained presentation"):
		return {}
	if not require(
		renderer.FindRoadSurfaceHit(Vector2(50.0, 0.0), 0.0).is_empty(),
		"Pre-commit supersession still exposed its retained surface"):
		return {}

	if not require(
		probe.CompletePreCommitTokenSupersession(),
		"Pre-commit replacement token did not publish"):
		return {}
	var recovered := presentation_token(renderer, "Pre-commit replacement")
	if recovered.is_empty():
		return {}
	if not require(
		...
		renderer.GetRoadMeshVertexCount() > retained_vertex_count and
		renderer.GetNodeMarkerCount() > retained_marker_count,
		"Pre-commit replacement did not atomically publish its first attempt"):
		return {}
	if not require_surface_hit(
		renderer,
		Vector2(50.0, 900.0),
	...
	print(
		(
			"UPDATE_TOKEN_PRECOMMIT_SUPERSESSION_RESULT " +
			"resource_before=%d resource_after=%d trigger_count=%d " +
			"superseded_attempt=1 replacement_attempt=1"
		) % [
			resource_count_before,
			int(probe.GetObjectResourceCount()),
			int(probe.GetPreCommitTokenSupersessionCount()),
		])
	return recovered

func require_update_token_precommit_road_style_supersession(
	renderer: Node,
	builder: Node,
	before: Dictionary
) -> Dictionary:
	var probe_script: Script = load(UPDATE_TOKEN_FAILURE_PROBE_PATH)
	if not require(probe_script != null, "Debug pre-commit road-style probe did not load"):
		return {}
	var probe: RefCounted = probe_script.new()
	if not require(probe != null, "Debug pre-commit road-style probe did not instantiate"):
		return {}
	...
	if not require(
		bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Update-token pre-commit road-style supersession probe did not arm"):
		return {}

	if not require(
		builder.BeginPlace(Vector2(0.0, 1000.0)),
		"Pre-commit road-style supersession mutation did not begin"):
		return {}
	builder.UpdatePlace(Vector2(100.0, 1000.0))
	if not require(
		builder.CommitPlace(Vector2(100.0, 1000.0)),
		"Pre-commit road-style supersession mutation did not commit"):
		return {}
	await process_frame
	await process_frame
	...
	if not require(
		...
		str(pending.get("failureType", "")).is_empty() and
		str(pending.get("failureMessage", "")).is_empty(),
		"Pre-commit road-style supersession did not retain the old presentation as pending"):
		return {}
	if not require_ordinary_change(before, superseded):
		return {}
	if not require(
		require_same(superseded, replacement, [
			...
			"graphFacadeGeneration",
			"changeSequence",
		], "Pre-commit road-style supersession") and
		int(replacement.roadStyleRevision) == int(superseded.roadStyleRevision) + 1 and
		int(replacement.renderRequestID) == int(superseded.renderRequestID) + 1,
		"Pre-commit road-style supersession did not advance style and request identities"):
		return {}
	if not require(
		int(pending.get("attemptCount", -1)) == 0 and
		...
		int(probe.GetPreCommitSupersededAttemptNumber()) == 1 and
		not bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Pre-commit road-style supersession was not raised inside the first target attempt"):
		return {}
	if not require(
		int(probe.GetObjectResourceCount()) == resource_count_before and
		int(pending.get("surfacePrimitiveCount", -1)) == 0 and
		...
		renderer.GetRoadMeshVertexCount() == retained_vertex_count and
		renderer.GetNodeMarkerCount() == retained_marker_count,
		"Pre-commit road-style supersession leaked resources or replaced retained presentation"):
		return {}
	if not require(
		renderer.FindRoadSurfaceHit(Vector2(50.0, 0.0), 0.0).is_empty(),
		"Pre-commit road-style supersession still exposed its retained surface"):
		return {}

	if not require(
		probe.CompletePreCommitTokenSupersession(),
		"Pre-commit road-style replacement token did not publish"):
		return {}
	var recovered := presentation_token(renderer, "Pre-commit road-style replacement")
	if recovered.is_empty():
		return {}
	if not require(
		recovered == replacement and
		...
		renderer.GetRoadMeshVertexCount() > retained_vertex_count and
		renderer.GetNodeMarkerCount() > retained_marker_count,
		"Pre-commit road-style replacement did not atomically publish its first attempt"):
		return {}
	if not require_surface_hit(
		renderer,
		Vector2(50.0, 1000.0),
		recovered,
		"Pre-commit road-style replacement"):
		return {}
	print(
		(
			"UPDATE_TOKEN_PRECOMMIT_STYLE_SUPERSESSION_RESULT " +
			"resource_before=%d resource_after=%d trigger_count=%d " +
			"superseded_attempt=1 replacement_attempt=1"
		) % [
			resource_count_before,
...
func require_update_token_precommit_scene_generation_supersession(
	renderer: Node,
	builder: Node,
	before: Dictionary
) -> Dictionary:
	var probe_script: Script = load(UPDATE_TOKEN_FAILURE_PROBE_PATH)
	if not require(probe_script != null, "Debug pre-commit scene-generation probe did not load"):
		return {}
	var probe: RefCounted = probe_script.new()
	if not require(probe != null, "Debug pre-commit scene-generation probe did not instantiate"):
		return {}
	...
	if not require(
		bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Update-token pre-commit scene-generation supersession probe did not arm"):
		return {}

	if not require(
		builder.BeginPlace(Vector2(0.0, 1100.0)),
		"Pre-commit scene-generation supersession mutation did not begin"):
		return {}
	builder.UpdatePlace(Vector2(100.0, 1100.0))
	if not require(
		builder.CommitPlace(Vector2(100.0, 1100.0)),
		"Pre-commit scene-generation supersession mutation did not commit"):
		return {}
	await process_frame
	await process_frame
	...
	if not require(
		...
		str(pending.get("failureType", "")).is_empty() and
		str(pending.get("failureMessage", "")).is_empty(),
		"Pre-commit scene-generation supersession did not retain the old presentation as pending"):
		return {}
	if not require_ordinary_change(before, superseded):
		return {}
	if not require(
		require_same(superseded, replacement, [
			...
			"changeSequence",
			"roadStyleRevision",
		], "Pre-commit scene-generation supersession") and
		int(replacement.sceneGeneration) == int(superseded.sceneGeneration) + 1 and
		int(replacement.renderRequestID) == int(superseded.renderRequestID) + 1,
		"Pre-commit scene-generation supersession did not advance scene and request identities"):
		return {}
	if not require(
		int(pending.get("attemptCount", -1)) == 0 and
		int(probe.GetPreCommitTokenSupersessionCount()) == trigger_count_before + 1 and
		int(probe.GetPreCommitSupersededAttemptNumber()) == 1 and
		not bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Pre-commit scene-generation supersession was not raised inside the first target attempt"):
		return {}
	if not require(
		int(probe.GetObjectResourceCount()) == resource_count_before and
		int(pending.get("surfacePrimitiveCount", -1)) == 0 and
		...
		renderer.GetRoadMeshVertexCount() == retained_vertex_count and
		renderer.GetNodeMarkerCount() == retained_marker_count,
		"Pre-commit scene-generation supersession leaked resources or replaced retained presentation"):
		return {}
	if not require(
		renderer.FindRoadSurfaceHit(Vector2(-50.0, 100.0), 0.0).is_empty(),
		"Pre-commit scene-generation supersession still exposed its retained surface"):
		return {}

	if not require(
		probe.CompletePreCommitTokenSupersession(),
		"Pre-commit scene-generation replacement token did not publish"):
		return {}
	var recovered := presentation_token(renderer, "Pre-commit scene-generation replacement")
	if recovered.is_empty():
		return {}
	if not require(
		recovered == replacement and
		...
		renderer.GetRoadMeshVertexCount() > retained_vertex_count and
		renderer.GetNodeMarkerCount() > retained_marker_count,
		"Pre-commit scene-generation replacement did not atomically publish its first attempt"):
		return {}
	if not require_surface_hit(
		renderer,
		Vector2(50.0, 1100.0),
		recovered,
		"Pre-commit scene-generation replacement"):
		return {}
	print(
		(
...
func require_update_token_precommit_change_sequence_supersession(
	renderer: Node,
	builder: Node,
	before: Dictionary
) -> Dictionary:
	...
	if not require(
		probe_script != null,
		"Debug pre-commit change-sequence probe did not load"):
		return {}
	var probe: RefCounted = probe_script.new()
	if not require(
		probe != null,
		"Debug pre-commit change-sequence probe did not instantiate"):
		return {}
	...
	probe.ArmPreCommitChangeSequenceSupersession(
		renderer,
		Vector2(0.0, 1400.0),
		Vector2(100.0, 1400.0))
	var trigger_count_before := int(probe.GetPreCommitTokenSupersessionCount())
	if not require(
		bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Update-token pre-commit change-sequence supersession probe did not arm"):
		return {}

	if not require(
		builder.BeginPlace(Vector2(0.0, 1300.0)),
		"Pre-commit change-sequence supersession target mutation did not begin"):
		return {}
	builder.UpdatePlace(Vector2(100.0, 1300.0))
	if not require(
		builder.CommitPlace(Vector2(100.0, 1300.0)),
		"Pre-commit change-sequence supersession target mutation did not commit"):
		return {}
	await process_frame
	await process_frame
	...
	if not require(
		...
		str(pending.get("failureType", "")).is_empty() and
		str(pending.get("failureMessage", "")).is_empty(),
		"Pre-commit change-sequence supersession did not retain the old presentation as pending"):
		return {}
	if not require_ordinary_change(before, superseded):
		return {}
	if not require(
		require_same(superseded, replacement, [
			...
			"graphFacadeGeneration",
			"roadStyleRevision",
		], "Pre-commit change-sequence supersession") and
		int(replacement.changeSequence) == int(superseded.changeSequence) + 1 and
		int(replacement.renderRequestID) == int(superseded.renderRequestID) + 1,
		"Pre-commit change-sequence supersession did not advance graph and request identities"):
		return {}
	if not require(
		int(pending.get("attemptCount", -1)) == 0 and
		int(probe.GetPreCommitTokenSupersessionCount()) == trigger_count_before + 1 and
		int(probe.GetPreCommitSupersededAttemptNumber()) == 1 and
		not bool(probe.IsPreCommitTokenSupersessionArmed()),
		"Pre-commit change-sequence supersession was not raised inside the first target attempt"):
		return {}
	if not require(
		int(probe.GetObjectResourceCount()) == resource_count_before and
		int(pending.get("surfacePrimitiveCount", -1)) == 0 and
		...
		renderer.GetRoadMeshVertexCount() == retained_vertex_count and
		renderer.GetNodeMarkerCount() == retained_marker_count,
		"Pre-commit change-sequence supersession leaked resources or replaced retained presentation"):
		return {}
	if not require(
		renderer.FindRoadSurfaceHit(Vector2(50.0, 1300.0), 0.0).is_empty() and
		renderer.FindRoadSurfaceHit(Vector2(50.0, 1400.0), 0.0).is_empty(),
		"Pre-commit change-sequence supersession exposed an unpresented surface"):
		return {}

	if not require(
		probe.CompletePreCommitTokenSupersession(),
		"Pre-commit change-sequence replacement token did not publish"):
		return {}
	var recovered := presentation_token(
		renderer,
		"Pre-commit change-sequence replacement")
	if recovered.is_empty():
		return {}
	if not require(
		recovered == replacement and
		int(renderer.GetPresentationState().get("attemptCount", 0)) == 1 and
		int(probe.GetPreCommitTokenSupersessionCount()) == trigger_count_before + 1 and
		renderer.GetRenderedEdgeCount() == retained_edge_count + 2 and
		renderer.GetRoadMeshVertexCount() > retained_vertex_count and
		renderer.GetNodeMarkerCount() > retained_marker_count,
		"Pre-commit change-sequence replacement did not atomically publish both mutations"):
		return {}
	if not require_surface_hit(
		renderer,
		Vector2(50.0, 1300.0),
		recovered,
		"Pre-commit change-sequence target mutation"):
		return {}
	if not require_surface_hit(
		renderer,
		Vector2(50.0, 1400.0),
		recovered,
		"Pre-commit change-sequence replacement mutation"):
		return {}
	print(
		(
